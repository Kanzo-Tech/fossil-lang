 ...
 *
 * The query layer for @fossil-lang/viewer: verb→SQL runs in WASM
 * (`fossil-graph-wasm`, single-source with the native runtime), and SQL
 * execution is delegated to a host-provided DuckDB-WASM `query` callback
 * (e.g. keasy's Mosaic coordinator). This is what makes the viewer
 * larger-than-RAM: the host's DuckDB streams Parquet over httpfs; the binding
