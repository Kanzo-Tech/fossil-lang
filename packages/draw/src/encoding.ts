 ...
 *
 * `apps/playground/src/Canvas.tsx` carried the answers as string literals: `fill="cluster_id"` on
 * the renderer, `field="birth_year"` on the chart, and `dense_id, birth_year, postcode, cluster_id`
 * in the crossfilter's view. Every one of them is a fact about *the bench corpus*, in a component
 * that claims to be a reader of any corpus — so pointing that app at a second one drew a blank
 * canvas and a chart over a column that is not there, with no error saying which of the four was
 * wrong.
 *
 ...
 *
 * A renderer that takes a radius per row would spend one on a column if it were handed one. It is
 * not: `Corpus.frame` carries **one** categorical per row — `Frame.categories`, from
 * `FrameParams.fill` — and no measure, so the streaming path has nothing to fill a size array from.
 * A baseline that loads the payload itself could, and a size ramp on one side of a comparison and
 * not the other is not a comparison. What would change it is a second column on `FrameParams` and a second
 ...
 * `fossil_sinks::manifest::Channel` has one statement in TypeScript, in `@fossil-lang/types` where
 * both of its readers can reach it, and a second one here would be a fifth spelling of a name this
 * file exists to stop spelling.
 */
export type { Channel } from '@fossil-lang/types';
 ...
 * door and this module no longer state the name independently — and that is why
 * `apps/playground/scripts/verify-encoding.mjs` no longer compares the two as if they were two
 * sources.
 *
...
/** What a reader draws one vertex type with. Every field is a column name or `null`. */
export interface Encoding {
   ...
   *
   * Handed to the renderer as the fill column, and reached again by a source that has to compose SQL
   * when a request names a CSS colour rather than a column.
   */
  readonly fill: string | null;
  /**
   * The column a vertex is **named** by — the subject IRI, when the payload carries one.
   *
   * Nothing draws it, and that is the measurement rather than an omission: carrying subjects to a
   * renderer costs 1.87× the tile — 8.016 compressed bytes per row against 9.23 for the four
   * columns a point is drawn from — so every renderer contract fossil has met makes them opt-in.
   * The drawing path carries addresses, and this says what would be asked for if something had to
   * be named.
   */
...
export interface EncodingParams {
   ...
   * function of two artefacts so that `apps/playground/scripts/verify-encoding.mjs` can run it in
   * Node over the same corpus the tab opens. {@link channelsFor} is the reader, and it is in this
   * file because a channel is an encoding and nothing else here has a use for one.
   */
  readonly channels?: readonly Channel[];
  /**
   * `graph.graph.yml`'s `quasi_identifiers`, as the `<Type>.<column>` pairs it writes them in.
   *
   * A `string[]` and not a parsed declaration, so that this module reaches no engine: the reader in
   * fossil's own tree is `apps/playground/src/bound.ts, readDeclaration`, which goes through
   * DuckDB, and a pure function is what `apps/playground/scripts/verify-encoding.mjs` can run in
   * Node against the same corpus the tab opens.
   */
 ...
 *
 * Exported because a source needs it and has no {@link Encoding}: a bounded source is built before
 * the corpus resolves and composes its SQL after, so it derives this from the corpus it opened
 * rather than taking a constant from its options.
 */
...
/**
 * What a reader draws `type` with, or `null` for a corpus that declares no vertex type at all.
 *
