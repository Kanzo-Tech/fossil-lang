...
//! (`adultos := users |> where(.edad >= 18)`, and `join` with it — ADR-0054)
//! makes `input` mean what it always said it meant: an index into the op list.
...
use crate::{read_source, render, resolve_source_uri};

/// The name the right side of a join carries its key under while the join is
/// being built. `on = .k` is `USING (k)`: the key appears **once** in the
/// result row (ADR-0054 §3), and DataFusion's join keeps both sides' keys — so
/// the right one is renamed out of the way and dropped after the join. The
/// `__fossil_` prefix is not a column any source can spell.
...
/// - a [`Op::Join`] whose `kind` is not `Inner` or whose `on` is not the
///   equality-by-name ADR-0054 admits;
...
pub async fn plan_relation(
    ...
    for i in evaluation_order(ops, index)? {
        let df = build(ctx, ops, i, &built, connections).await?;
        built.insert(i, df);
    }
    built
        .remove(&index)
...
async fn build(
    ...
    match &ops[index] {
        Op::Source {
            ...
            binding,
            ..
        } => read_source(ctx, &resolve_source_uri(uri, connections), format, binding).await,
        ...
        other => Err(DataFusionError::Plan(format!(
            "`{}` is defined in the operator algebra and this engine does not execute it \
             (ADR-0054 admits `Filter`, `Project` and `Join`/`Inner`)",
            op_name(other)
        ))),
    }
}

/// `fila(izq) ⊎ fila(der)` with the key identified once (ADR-0054 §3/§4).
///
...
fn join(
    ...
    if kind != JoinKind::Inner {
        return Err(DataFusionError::Plan(format!(
            "join kind `{kind:?}` is not executable: only `Inner` is (ADR-0054 §2 — an outer \
             join fabricates NULL, and no output shape can declare a nullable property yet)"
        )));
    }
    ...
    if !shared.is_empty() {
        return Err(DataFusionError::Plan(format!(
            "`{left_name}` and `{right_name}` both carry {shared:?}, and a join identifies only \
             the key `{key}` (ADR-0054 §4) — the checker refuses this before it gets here"
        )));
    }
...
fn join_key(on: &Expr<'_>, left_name: &str, right_name: &str) -> datafusion::error::Result<String> {
    let Expr::BinOp {
        ...
        return Err(DataFusionError::Plan(format!(
            "a join condition is an equality by name (`on = .k` ≡ `USING (k)`, ADR-0054 §3); \
             this one is {}",
            expr_name(on)
        )));
    };
    let (
        ...
        return Err(DataFusionError::Plan(format!(
            "a join condition equates two column references (`on = .k`, ADR-0054 §3); this one \
             equates {} and {}",
            expr_name(lhs),
            expr_name(rhs)
        )));
    ...
    if lc != rc {
        return Err(DataFusionError::Plan(format!(
            "a join key is one name on both sides (`on = .k` ≡ `USING (k)`, ADR-0054 §3); this \
             one names `{lc}` and `{rc}`. Two keys with different names are the extension \
             ADR-0054 declares and does not build"
        )));
    }
