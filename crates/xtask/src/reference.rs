...
/// Where the prompt's library section goes, repo-relative.
pub const PROMPT_LIBRARY: &str = "packages/prompt/src/catalogue.generated.ts";

/// The catalogue as a model reads it — `@fossil-lang/prompt`'s library section.
///
/// The same parse, the same [`signature`] and the same [`call_spelling`] the
/// reference page prints, so a name the prompt offers is a name the checker
/// accepts. The hand-written prompt it replaces named two namespaces, `validate`
/// and `anon`, that no row declares.
///
/// # Panics
///
/// If a row cannot be serialised as a JSON string, which cannot happen.
#[must_use]
pub fn emit_prompt_library(cat: &Catalogue) -> String {
    let mut lines: Vec<String> = vec![
        "## Library".into(),
        String::new(),
        "Every name a program may call. A name not listed here does not exist.".into(),
        String::new(),
        "### Readers — `io.`".into(),
        String::new(),
    ];
    for row in &cat.rows {
        let extensions: Vec<String> = row.extensions.iter().map(|e| format!(".{e}")).collect();
        let what = match (&row.call, &row.decodes) {
            (Some((sig, _)), _) => format!("`io.{}{}`", row.name, signature(sig)),
            (None, Some(_)) => format!(
                "`io.{}(uri: String)` — a shape document, bound as `type {{ … }} := io.{}(\"…\")`",
                row.name, row.name
            ),
            (None, None) => format!("`io.{}`", row.name),
        };
        lines.push(format!("- {what} — reads {}", extensions.join(" ")));
    }
    for (head, entries) in by_head(cat) {
        lines.push(String::new());
        if is_namespace_head(head) {
            lines.push(format!("### `{head}.`"));
        } else {
            lines.push(format!(
                "### `{head}.` — also written on the value: `x.f(…)` is `{head}.f(x, …)`"
            ));
        }
        lines.push(String::new());
        for e in entries {
            lines.push(format!("- `{}{}`", e.name, signature(&e.sig)));
        }
    }
    let names: Vec<String> = cat
        ...
        .iter()
        .map(|f| f.name.clone())
        .chain(cat.rows.iter().map(|r| format!("io.{}", r.name)))
        .map(|n| serde_json::to_string(&n).expect("a string serialises"))
        .collect();
    let body: Vec<String> = lines
        .iter()
        .map(|l| format!("  {},", serde_json::to_string(l).expect("a string serialises")))
        .collect();
    format!(
