...
//!   2. `textDocument/definition`       → a cross-file `Location`
...
//!
//! Both canonical fixtures are opened so the cross-file goto-def (workspace =
//! open files, ADR-0023) has a target. All frames are written upfront, then
//! stdin is dropped and stdout drained — the pattern from `lsp_hover_smoke.rs`.
...
/// Build + cache the `fossil-lsp` binary path (build via `cargo build`, NOT
/// `cargo test`, so build errors surface distinctly — checker Warning W4).
fn fossil_lsp_binary() -> &'static PathBuf {
    ...
    BIN.get_or_init(|| {
        ...
        assert!(
            bin.exists(),
            "fossil-lsp binary not found at {}",
            bin.display()
        );
        bin
    })
...
const URI_A: &str = "file:///tmp/canonical_200.fossil";
const URI_B: &str = "file:///tmp/canonical_200_b.fossil";
...
fn fixture_b() -> String {
    std::fs::read_to_string(repo_root().join("tests/fixtures/canonical_200_b.fossil"))
        .expect("read canonical_200_b.fossil")
}
...
fn lsp_serves_all_six_capabilities_over_the_transport() {
    ...
    // hover: inside the IRI template of mapping #1 (Person). The template
    // synthesises an `IriTemplate` ty without needing a CSVW schema (the
    // source-side `.field` path requires a sibling .csvw, which the fixture
    // intentionally omits — it runs under AcceptAll).
    let (hover_l, hover_c) = pos_of(&src_a, "iri = `${ex:}person/${.id}`", 9);
    // definition: cursor on the `ex` prefix use in the sibling file B (declared
    // in file A) — cross-file goto-def.
    let (def_l, def_c) = pos_of(&src_b, "Event : ex:Event from events", 9);
    // completion: just after `clean.` in file A would need an edit; instead probe
    // an identifier position — completion is unconditional (stdlib + prefixes).
    let (comp_l, comp_c) = pos_of(&src_a, "foaf:name = clean.trim(.name)", 13);
    // codeAction range: over the first property of mapping #1.
    let (ca_l, ca_c) = pos_of(&src_a, "foaf:name", 0);

    {
        ...
        stdin
            ...
            .unwrap();
        stdin.write_all(did_open(URI_A, &src_a).as_bytes()).unwrap();
        stdin.write_all(did_open(URI_B, &src_b).as_bytes()).unwrap();

        stdin
            .write_all(req(2, "textDocument/hover", text_pos(URI_A, hover_l, hover_c)).as_bytes())
            .unwrap();
        stdin
            .write_all(req(3, "textDocument/definition", text_pos(URI_B, def_l, def_c)).as_bytes())
            .unwrap();
        stdin
            .write_all(
                req(
        ...
        // 2. definition → a cross-file Location array pointing at file A.
        let def = by_id(3);
        let locs = def
            .pointer("/result")
        ...
        assert!(
            locs.iter()
                .any(|l| { l.pointer("/uri").and_then(serde_json::Value::as_str) == Some(URI_A) }),
            "cross-file goto-def must point at file A (where `ex` is declared); got {locs:?}"
        );
        ...
        // The published diagnostics for the clean fixtures must be empty.
        let diags_a: Vec<&serde_json::Value> = frames
            .iter()
            .filter(|m| {
                m.get("method").and_then(serde_json::Value::as_str)
                    == Some("textDocument/publishDiagnostics")
                    && m.pointer("/params/uri").and_then(serde_json::Value::as_str) == Some(URI_A)
            })
            .collect();
        assert!(
            !diags_a.is_empty(),
            "expected a publishDiagnostics for file A"
        );
        for d in diags_a {
            let arr = d
