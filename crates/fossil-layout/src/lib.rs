...
//! library wrote — `docs/design/one-engine.mdx` has the argument. The crate does
//! still link `DataFusion`, transitively through `fossil-df`, for the edge
//! below.
//!
...
//! `fossil_df::files::TileWriter`, which is now the **only** writer of the
//! corpus's payload — so that the row-group-per-tile property a reader indexes
//! on is stated in one place. It shared that job with `batches_to_parquet`,
//! which wrote the staged files this pass used to read back; nothing stages any
//! more. That is a corpus crate depending on an engine crate, and both obvious
