...
Choosing what goes in the neutral vocabulary is the design work, and two things earn their place.
...
**Closure, which has three states and not two.** Whether a node may carry predicates the shape does
not mention is a property of the *shape*, and it is tempting to model it as a bit:

| state | a node may carry | what a boolean loses |
| --- | --- | --- |
| `closed` | only the predicates the shape names | nothing — the bit keeps this one |
