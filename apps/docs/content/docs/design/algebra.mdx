...
What the checker can say and the engine cannot is **which side goes unmentioned**. A `JoinSide`
carries one relation name; the scope here carries every binding a body may address, which is what
makes `` `User` is never named `` a sentence rather than a guess.
...
- **How a pipeline's type is reported in the editor.** A source binding today has a schema from a
  ...
  the derived one.
- **A join whose left side is itself a join.** `Both := L.join(R, …)` followed by
  `Tri := Both.join(T, on = R.k == T.k)` passes `fossil check` — it is a join by the rule above,
  `R` being a binding of the left side — and then fails to plan: *«the left side of the join
  condition qualifies a column with `R`, which is neither input (`Both`, `T`)»*. The two layers
  disagree about what a side IS. `fossil_mir::JoinSide` carries the one name the relation answers
  to, which after a join is the pipeline's; the checker's scope carries every binding, because that
  is what a mapping body may write. The seam is the `JoinSide`, and the fix is a set of names rather
  than a name — which is a change to what the engine reads, so it arrives with the engine and not
  before.
