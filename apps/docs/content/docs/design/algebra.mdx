...
is where an operator set stops being a list and starts being an algebra. Seven of the fourteen are
what the lowering emits today; the other seven are defined and unreached, which is a different
problem from being over-designed and takes a different fix:

```mermaid
graph LR
  SRC["Source"] --> FIL["Filter"] --> PRJ["Project"] --> JOI["Join"]
  JOI --> EV["EmitVertex"] --> SNK["Sink"]
  JOI --> EE["EmitEdge"] --> SNK
  WAIT["<i>defined, not yet reached</i><br/>Extend · Rename · Union · GroupBy<br/>Aggregate · Distinct · Empty"]
