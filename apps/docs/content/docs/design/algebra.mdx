...
description: Thirteen operators over typed schemas, ten of which the lowering emits today, and the six relational verbs that reach them. A conservative typed elaboration of a published operational semantics — and the plan is DataFusion's.
---
...
| `GroupBy` | the grouping columns, plus one column per aggregate, whose type the aggregate decides | `SELECT keys, aggs … GROUP BY keys` |
...
is where an operator set stops being a list and starts being an algebra. Ten of the thirteen are
what the lowering emits today; the other three are defined and unreached, which is a different
problem from being over-designed and takes a different fix:

```mermaid
graph LR
  SRC["Source"] --> FIL["Filter"] --> PRJ["Project"] --> JOI["Join"]
  JOI --> DIS["Distinct"] --> UNI["Union"] --> GRP["GroupBy"]
  GRP --> EV["EmitVertex"] --> SNK["Sink"]
  GRP --> EE["EmitEdge"] --> SNK
  WAIT["<i>defined, not yet reached</i><br/>Extend · Rename · Empty"]
...
equi-join (`crates/fossil-df/tests/pipeline.rs, a_join_whose_two_keys_are_named_differently_plans`), a compound key is a conjunction of equalities
joined by `and` (`crates/fossil-df/tests/pipeline.rs, a_compound_key_joins_on_both_columns`), and a self-join tells its two halves
apart by the alias `X as Y` (`crates/fossil-df/tests/pipeline.rs, a_self_join_keeps_both_sides_apart_by_their_alias`).

#### And the checker is where it is exacted

Two questions have to be answered about a condition and they are answered in two places, one line
apart. **Is it a condition** — a `Bool` over columns that exist under the bindings that qualify
them — is `fossil_hir::infer::typecheck_stage`, and it is the same expression typer a mapping body
uses, which is the whole of why `Row.celsius > "abc"` stopped being legal in a `where` and illegal
four lines below. **Does it relate these two sides** is `fossil_hir::infer::check_join_condition`,
and it is a different question with a different answer: `on = Purchase.id == Purchase.user_id` is a
perfectly good `Bool` over columns that all exist, and it describes the product of the two relations
with a filter on one of them.

The rule was not invented at that layer. `fossil_df::plan::join_equalities` has enforced it since
the day joins executed, and it keeps its copy, because a MIR reaching the engine need not have come
through this checker. What moved is *where an author meets it*: the refusal now carries a span and
the author's own names, and [the sixth
diagnostic](/docs/book/diagnostics#6-a-join-that-names-one-side-twice) is the program that produces
it.

What the checker can say and the engine cannot is **where** — a span over the author's own text.
By the time a `JoinSide` reaches the engine the program is a plan, which is what makes
`` `User` is never named `` a sentence at a source location rather than a `DataFusionError` at
`fossil run`.

#### A side is a set of names, because a join composes

**A join's result is addressed by both its sides' bindings**, and that is what lets the body write
`Purchase.amount` beside `User.email`. It is also what lets a join key the next one:

<Program src="chained-join/chained-join.fossil" region="chain" title="chained-join.fossil" />

`User` qualifies a column of `Both` because `Both` is a join and **a join re-qualifies neither
side** — the one composite operator that does not. `Op::Union` re-qualifies wholly (DataFusion
returns its schema with no qualifier at all, so there is nothing to keep) and `Op::GroupBy` does it
to the aggregates; only those two put a pipeline's own name on a column.

So `JoinSide` carries `relations`, a set, and it carried a name until 2026-08-28. The lowering filled
that name with the pipeline's after a join, and the pipeline's name qualified nothing: the engine
refused `User` as *«neither input (`Both`, `Region`)»* while `Both` addressed no column of either
side. The rule was not a narrower one than the checker's but an **inverted** one — it named as the
alternative the single spelling that could not resolve, and refused the only spelling that could.
Two programs `fossil check` accepted could not be planned, and the second had been described here
as working since this page was written: `Purchase.join(Adults, …)`, where `Adults := User.where(…)`
is addressed as `User` because a filter renames nothing.

Both are the program above, and it is in the conformance set. What would have written the seam into
the language instead — refusing the chained join in the checker — is in
[`design/discarded`](/docs/design/discarded) with what would bring it back, which is nothing.
...
`crates/fossil-df/tests/pipeline.rs, distinct_over_a_union_drops_the_shared_row` unions three names onto three and counts six, then five.
...
- **The thirteen operators stay.** They are the pipeline's vocabulary, not an aspiration.
...
express**: `Op` has thirteen variants (`crates/fossil-mir/src/op.rs`) and stops there, because an
...
- **The six relation verbs the catalogue declares and the lowering does not implement.** `seq/`
  has twelve rows — `map`, `flatten`, `take`, `drop`, `sort` and `count` beside the six — and the
  refusal says so by reading the table rather than by listing what it knows. Each one that arrives
  has to arrive with the operator that carries it and a program that spells it, not as a row.
  `seq.aggregate` was a seventh and is deleted: `group_by` took the aggregations into itself,
  because a grouping with nothing aggregated has the rows of `select(…).distinct()`.
- **The three join flavours that are not `Inner`.** They stay defined and unreachable
  (`crates/fossil-df/tests/pipeline.rs, an_outer_join_is_refused_by_name` pins the refusal). An outer join manufactures NULL, and
...
- **How a pipeline's type is reported in the editor.** A source binding today has a schema from a
  ...
  the derived one.
- **Which side a bare column belongs to, when two bindings of one relation carry the same name.**
  Nothing in the surface spells a bare column, so this is not reachable by writing a program today;
  `fossil_df::plan::column` builds an unqualified reference for one anyway, and DataFusion's rule
  for it — resolve against whichever relation has the name, and error naming both candidates if two
  do — is inherited rather than chosen. The day a verb produces a column with no binding to
  qualify it, that inheritance becomes a decision.
