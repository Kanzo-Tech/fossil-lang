...
The browser turned out to need **two** wires rather than one, and the rule held for the second
exactly as written. `postMessage` is a transport with a client on the other end of it, which a page
that already calls the checker in-process has no reason to build in order to ask what type is under a
cursor — so hover, completion and definition are also plain method calls on the wasm workspace. One
implementation still, two projections of it, and a guard that sweeps every cursor of a program
comparing what the two wires say rather than comparing either against a written-down answer.
...
discriminant a row carries, and a host keys on `"Comment"` rather than on `2`; `semanticLegend()`
already did this for LSP semantic tokens, and completion kinds followed — LSP spells
