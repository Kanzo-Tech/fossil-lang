...
`DESCRIBE SELECT * FROM read_csv_auto(…)` per source, run by the host over the sources the def map
bound and before the check (`crates/fossil-introspect/src/lib.rs, pre_introspect_and_register`) and cached per source. The programs are small and
...
of the document the program names (`crates/fossil-hir/src/shapes.rs, resolve_target_shape`). Which schema language
...
(`crates/fossil-hir/src/check.rs, resolve_predicate`).
...
holds. `compatible` (`crates/fossil-hir/src/check.rs, compatible`) checks them where they meet, before the
...
(`crates/fossil-hir/src/check.rs, check_required_properties`):
...
[the typing rules](/docs/design/typing) — and `compatible` runs them on every property.
</Callout>
...
- **The upper bound of a cardinality.** `check_required_properties` observes a required property
  ...
  `constraint.occurs` (`crates/fossil-hir/src/check.rs, check_property`). Nothing in the language constructs a
