...
exists (`crates/fossil-introspect/src/lib.rs:318`) and cached per source. The programs are small and
...
of the document the program names (`crates/fossil-hir/src/shapes.rs:525`). Which schema language
...
(`crates/fossil-hir/src/check.rs:472`).
...
holds. `compatible` (`crates/fossil-hir/src/check.rs:901`) checks them where they meet, before the
...
(`crates/fossil-hir/src/check.rs:595`):
...
- **The upper bound of a cardinality.** `check_required_properties` observes a required property
  ...
  `constraint.occurs` (`crates/fossil-hir/src/check.rs:449`). Nothing in the language constructs a
