...
`openCorpus`'s `node(id)` takes the subject IRI. The alternative was the `dense_id`, and it is the
...
`openCorpus` is the layer that has an engine to read them with,
...
- **Its addressing model is this one.** A metadata document plus chunks addressed by a key computed
  from the index, no server, store-agnostic. That is the engine-free depth of `openCorpus`, spelled
...
fossil publishes is the address — `openCorpus` with no engine lent to it, one reader, no bytes — and
...
graph LR
  ...
  C --> WS["the same plan · wasm32, usize 32<br/>re-entered from JS as openCorpus"]
  JS --> D{"diff"}
  RS --> D
  WS --> D
