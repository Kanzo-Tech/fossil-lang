...
<Callout type="info">
  ...
  `conformance/expected.json` was the commit after it, in which `resolveCorpus` stopped being a
  second implementation of `fossil_graph::plan` and [became a binding over
...
fossil publishes is the address — `resolveCorpus`, one reader, no bytes — and what draws is
...
graph LR
  ...
  C --> WS["the same plan · wasm32, usize 32<br/>re-entered from JS as resolveCorpus"]
  JS --> D{"diff"}
  RS --> D
  WS --> D
...
Then the second JavaScript one went: `resolveCorpus` was a thousand lines that agreed with the Rust
because people kept making it agree, nothing compared the two, and it is a binding over
