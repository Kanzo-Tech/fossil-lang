...
`@fossil-lang/corpus/address` is synchronous and reads no bytes, so it cannot answer a rectangle:
`tilesFor` takes tile numbers, and the host supplies them from the per-tile `x`/`y` statistics
Parquet writes into every footer. `openCorpus` is the layer that has an engine to read them with,
...
<Callout type="info">
  ...
  moved `packages/graph/` to `packages/corpus/` and touched no arithmetic. `fossil_graph::plan`
...
finite box, `levelFor` picks the level, `view` answers it, and what is left in the file is the
mapping between two vocabularies. Nothing in it knows how cosmos.gl holds a buffer, and nothing in
the renderer knows what a corpus is.
...
graph LR
  ...
  C --> RS["fossil_graph::plan · native and wasm32"]
  JS --> D{"diff"}
  TS --> D
  RS --> D
