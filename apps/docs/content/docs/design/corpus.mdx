...
`@fossil-lang/corpus/address` states its own seam: *"**No boxes.** Which tiles a rectangle touches
...
numbers back here."* `tilesFor` takes `tiles: number[]` — the caller must already know.
...
`dense_id` range is a run of tiles — arithmetic, on the manifest's `chunk_size` alone, with no
footer, no reader and no engine. The sentence was true before the renumbering and nobody revisited
it after.

What this changes is not an optimisation, it is which package can answer:

- **A camera with no engine can address itself.** Today the browser must hold a Parquet reader to
  ask "which tiles"; the coarse levels of a pyramid have no reason to. The subpath's promise — no
  fetch, no WASM, no promise — currently stops one step short of the question a camera actually
  asks.
- **`openCorpus` stops being the only door to a picture.** It keeps the engine for what needs one.

<Callout type="warn" title="Arithmetic over-covers, and the footers are what prune it exactly">
A Morton decomposition of a box returns a **superset**: the curve enters and leaves the rectangle,
so some addressed tiles hold nothing inside it. The footer statistics prune that exactly, which is
what they are for and why they do not go away. The two compose — arithmetic to address, footers to
refine where a reader exists — and the cost of the arithmetic-only path is measurable against the
1.13× over-read the footer path already records. **It is not written down until it is measured.**
</Callout>
...
section exists so the next person argues against the right thing, and the measurement that would
decide the top half is the one the callout above asks for.
