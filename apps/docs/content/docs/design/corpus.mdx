...
to take the payload vocabulary off the manifest at all, because `property_groups` enumerates three
properties where `DESCRIBE` sees seven columns. A flag on a list that does not enumerate the
...
- **The manifest declares the numbers.** A level list is `log4(V / chunk_size)` integers, so nothing
  here grows with the graph. And which levels a writer spent bytes on is a **policy**, not a
  derivation — a reader recomputing it from `vertex_count` would be reimplementing the writer and
  would 404 the day the policy moved. One function chooses them, and both the pass that writes the
  files and the caller that declares them call it.

Addressing a level needs nothing new. Level *k*'s row *i* is the payload row with `dense_id = i·4^k`,
so a level tile is a contiguous run of payload tiles, addressed by the payload's own shift plus `2k`.
</Callout>
...
`crates/fossil-sinks/src/manifest.rs, EdgeLevels` and
