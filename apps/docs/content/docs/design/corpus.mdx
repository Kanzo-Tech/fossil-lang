...
Three things about the pyramid are decided, and each is spelled where it lives —
`crates/fossil-sinks/src/manifest.rs, VertexLevels` and
`crates/fossil-layout/src/layout.rs, write_levels`.

- **Three levels, and the coarsest is the one that fits a single tile.** Coarser buys nothing: one
  tile is already one range request. Each level is one 2× zoom step, and the finest holds about
  4·`chunk_size` rows — the order of a camera's point budget — so a view wanting more points is a
  view zoomed in far enough that striding the payload reads only its own window's tiles. The cost is
  therefore `2³−1` tiles' worth of rows **whatever V is**: measured over 300,000 vertices at the real
  4,096-row tile, 74 tiles and 18,122 kB of payload against **589 / 298 / 153 kB** of levels — 1,040
  kB, 5.7% of the type, and falling as the corpus grows.
- **The floor is expressed in tiles, and set high on purpose.** Under 64 tiles a type gets no
  pyramid. The asymmetry is the argument: a corpus below the floor draws the same picture off the
  predicate and only reads more, where a floor set too low charges every small corpus bytes for a
  view it already serves in a handful of requests. `fossil run examples/hello.fossil` is three orders
  of magnitude under it and writes nothing.
- **The manifest declares the numbers**, which departs from `codes:` deliberately. A code anchor is
  two `u32` per tile and grows with the corpus; a level list is at most three integers at any V. And
  which levels a writer spent bytes on is a **policy**, not a derivation — a reader recomputing it
  from `vertex_count` would be reimplementing the writer and would 404 the day the policy moved. One
  function chooses them, and both the pass that writes the files and the caller that declares them
  call it.

Addressing a level needs nothing new. Level *k*'s row *i* is the payload row with `dense_id = i·4^k`,
so a level tile is a contiguous run of payload tiles and the published code anchor already bounds it
in Morton space.
</Callout>
