...
Addressing a level needs nothing new. Level *k*'s row *i* is the payload row with `dense_id = i·2^k`,
...
positioned, so the edges a coarse view draws scale with the marks — `V/2^k · degree` — and not with
their square. **Level *k* of a relation is therefore the edges incident to a level-*k* vertex**,
`src_dense % 2^k = 0 OR dst_dense % 2^k = 0`, and each row carries **both endpoints' coordinates**
