...
## Conformance is round-trip, and the third leg is missing
...
The dotted leg is not built, so the three-way diff is a two-way one. What does run lives in
`apps/corpus/conformance/verify.mjs`, and the conventions it checks are documented for reader
