...
expected and accepted. There is still no tile store, no tile index and no tile emission anywhere in
`crates/`, and none of the six verbs answers a camera. **Decided:** the camera is addressed, not
queried. **Undone:** the tiles that addressing reads.
</Callout>
