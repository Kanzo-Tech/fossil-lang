...
<Callout type="warn" title="The edges get no pyramid, and not for the reason that rejects a centroid">
A level's **induced** edge set — both endpoints in it — nests exactly as its vertices do, so it is
not refused for a centroid's reason. It is refused because it is empty. A level keeps 1/2^*k* of the
vertices and an edge needs two of them, so it keeps on the order of 1/2^2*k* of the edges: measured
over a fixture of **27,976 edges** at mean degree 14, the three written levels induce **1, 0 and 0**.

So a coarse camera draws points and no links, which is not a new state for it to be in — `links` is
already 0 in a window whose edges leave the frame. The alternative is what this page refuses twice
over: an edge between two survivors standing in for a path through vertices nobody drew is synthetic,
and replacing it with the real path as the camera zooms moves every line on screen. Links come from
the payload's own `by_source` tiles, over the window, at the zoom where there are links to draw.
</Callout>
