...
`u32`, **1,960 B for a million vertices** against the 226 kB of Parquet footer the geometric path
reads over the same corpus — and readable by anything that can read a number. `TileCodes` is that
input, and the writer already holds it: `morton` and `order` are both in hand in the layout pass, so
publishing it is a manifest field and not a second computation. **Nothing publishes it yet**, which
is the outstanding half of this section rather than a limitation of the arithmetic.
...
already holds it. What that does **not** settle is the reversal condition, which is a reader and not
