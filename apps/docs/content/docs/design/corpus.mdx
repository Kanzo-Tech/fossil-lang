...
### A reader is handed the subject IRI, because an address is not something to key on

`open`'s `node(id)` takes the subject IRI. The alternative was the `dense_id`, and it is the
cheap one: a shift names the tile, one request answers, the whole lookup is `O(1)`. It is refused
because **the spatial order is the id space** — redoing the layout ranks every vertex by the Morton
code of its new position and lets that rank be its id, so a `dense_id` held anywhere outside the
corpus names a *different vertex* after the next write. A bookmark, a selection, a link from another
system and a row in somebody else's database all key on something, and an address is not something
to key on. `node` refuses a `BigInt` with a `TypeError` that says so.

What would reverse it is a corpus whose ids survive a re-layout — a stable id column written beside
the address. Nothing writes one, and adding one is a format change and not a reader change.

**The `is_primary` flag is not what this keys on, and no longer because anything disagrees.** The
guard `identity-is-the-subject` and the writer now both mark `subject`. The flag is simply not
*reachable*: `packages/corpus/src/manifest.ts` is a line scanner and `properties` is nested one
level deeper than anything it addresses, so getting at it means a third shape in `ScannedManifest`
— a sequence of mappings inside a mapping inside a sequence — which is the nesting that module
refuses by name, plus the same growth in `apps/corpus/guards/manifest.mjs`, a deliberate copy for a
third party who has neither this repository nor npm. And independently: the reader already refuses
to take the payload vocabulary off the manifest at all, because a payload projection's `properties`
enumerates three where `DESCRIBE` sees seven columns. A flag on a list that does not enumerate the
payload would name the identity of some corpora and be silent about others. The **column name** is
the stable thing, so the column name is what it keys on.
...
| vertex info | `type`, `chunk_size`, `prefix`, `version`; per property `name`, `data_type`, `is_primary`, `is_nullable` |
| edge info | `src_type`, `edge_type`, `dst_type`, `chunk_size`, `src_chunk_size`, `dst_chunk_size`, `directed`, `prefix`, `version`; per projection `ordered`, `aligned_by`, `file_type` |
...
Nine divergences follow from that rule. None of them is a bug.
...
| `chunk_size` on an edge | rows per edge chunk, recommended 2²² | equals `src_chunk_size` and is the addressing unit, so nothing bounds the row count (`crates/fossil-sinks/src/manifest.rs`) | `by_source/chunk{k}.parquet` holds every edge out of vertex tile `k`. Same key, different meaning: a reader taking the borrowed name at its borrowed value gets a number instead of an error |
| data-file paths | none, anywhere | `<type prefix><projection path>chunk{k}.parquet`, and `<vertex prefix><index prefix>tile{k}.parquet` for the one artefact that is not a projection | there is nothing here to diverge from |
...
| a projection entry with no `path` | prose names the prefix; the worked example omits it from all three `adj_lists` entries | reported absent rather than composed from convention | conformance case `unaddressable`. The specification's own example reads here as a corpus nobody can address |
| `projections` | `property_groups` on both infos and `adj_lists` on the edge — three blocks for the payload, the orientations and the columns each carries | one block, whose entries are `{ path, scale, aligned_by?, ordered?, file_type, properties }` (`crates/fossil-sinks/src/manifest.rs, Projection`) | the four blocks fossil had described one rule: a corpus is an order, a cut, and projections of that sequence, and the payload is the projection at `scale: 1`. A GraphAr reader looks for `property_groups` and `adj_lists` and finds neither, which makes this the largest of the nine — and it costs nothing that was not already spent on `uint32` |
...
### The version string names a format this page declares non-conformant

**Nine divergences up, and then `version: gar/v1`.** The paragraph above says this is not a
GraphAr-conformant corpus and does not claim to be; the manifest it describes opens with GraphAr's
own version string, which is a claim of exactly that. A reader who takes the string at its word
reaches for GraphAr's reader and it throws on `dense_id` — the first property of every corpus fossil
writes — which is the first row of the table above. **The string is the only line of a fossil
manifest that contradicts the page specifying it.**

That is the same defect as the one [the cell pyramid's old name](/docs/design/cells#the-name-promised-a-nesting-this-does-not-have-and-the-name-is-gone)
carried, and that one has been paid off: a borrowed name promising a contract the artefact does not
honour. It is not a tenth divergence —
a divergence is a place where fossil does something else on purpose, and this is a place where the
manifest says something untrue about all nine.

<Callout title="Nothing in this tree keys on it, and that is the fact the decision turns on">
`gar/v1` appears **69 times in 34 files**, and **not one of them reads it**. There is no comparison,
no `match`, no branch, no rejection path, no version gate — in the crates, in
`apps/corpus/conformance/corpus/graph.graph.yml` and its siblings, in the guards, or in the
TypeScript. `packages/executor/src/index.ts, GraphInfo` carries `version` as an opaque `string` and
nothing ever looks at it.

| what the 69 are | count | files |
| --- | --- | --- |
| a corpus or a fixture on disk | 27 | 25 |
| a test asserting the writer emitted the string | 10 | 6 |
| doc comments and fixture literals | 18 | 5 |
| `crates/fossil-sinks/src/manifest.rs, GRAPHAR_VERSION` and its five uses by the writer | 6 | 3 |
| the string hard-coded past that constant | 5 | 5 |
| prose on this site | 2 | 2 |
| **a reader that decides anything on it** | **0** | **0** |

**The five that bypass the constant are their own finding.** `crates/fossil-graph/src/manifest.rs`,
`crates/fossil-graph/src/executor.rs`, `crates/fossil-mcp/src/lib.rs` and two examples/tests beside
them write `"gar/v1"` as a literal rather than reading `GRAPHAR_VERSION`, so *changing the constant
does not change the string* in three crates. Whatever is decided, that is a defect today: there is
one name for the version and six places that state it.
</Callout>

So the compatibility question that made [the pyramid's rename](/docs/design/cells#the-name-promised-a-nesting-this-does-not-have-and-the-name-is-gone)
expensive **does not arise here**, and it is worth being precise about why. A manifest key is read
by name, so renaming one changes what a reader finds. A manifest *value* nobody reads is inert: an
old corpus keeps saying `gar/v1`, a new one says something else, and every reader in this tree
behaves identically on both because none of them looks. The cost of changing it is the cost of
rewriting a string in six write sites, 25 fixture files and 10 assertions — mechanical, and the
compiler and the test suite find all of it.

**What each option costs, and what it forecloses.**

| option | cost | what it forecloses |
| --- | --- | --- |
| **Keep `gar/v1`** | nothing | the manifest keeps making a claim this page denies, and the denial lives only here. A third party reading the corpus and not this page has no way to learn it |
| **A fossil string** — `fossil/v1`, or a version that carries the divergence set | six write sites, 25 fixtures, 10 assertions, and a paragraph on `/docs/format` saying what the string means | conformance as a *later* option: a corpus that once said `gar/v1` and now says `fossil/v1` has said two things, and a reader that cached the first is a reader this format has not thought about |
| **Keep the string and state the claim** — a second field naming what is borrowed | one new optional key, which is a format change with all of a format change's obligations | nothing, and it also fixes nothing: a reader that does not read `version` will not read its annotation either |

**What this needs in order to be taken.** Not a measurement — the measurement is above and it is
`0`. Two things:

1. **Whether the version string is for a reader or for a human.** Nothing reads it, so today it is
   for a human, and a string that is for a human and says the wrong thing has no defence. If it is
   ever to be for a reader, the thing to decide is not its spelling but whether a reader may reject
   a corpus on it — and [one clock, running one way](#the-artifact-is-the-api) says that decision is
   cheap exactly until a corpus reaches somebody who cannot regenerate it.
2. **The same ruling the cell pyramid waits on**, about whether two spellings of one thing may be
   legal at once. Answering it differently in the same manifest is the outcome to avoid.

The five hard-coded literals are worth fixing either way, and do not wait on any of this.
...
`crates/fossil-layout/src/layout/community.rs, community_hierarchy` and the Morton ranks in
`crates/fossil-layout/src/layout/morton.rs, morton_ranks` are `u32`
...
above zero. `cluster_id` is a colour and not a level. The directories that *are* levels are
`vertex/<Type>/l{k}/`, and what is in them is the decimation below rather than anything Louvain
built: real vertices at real positions, one per quadtree cell of depth *k*.
...
<Callout title="The levels are written, and they optimise the predicate rather than replace it">
Level *k* selected by `dense_id % 4^k` needs no bytes on disk: it is a predicate. What a written
`vertex/<Type>/l{k}/` buys is that a coarse camera reads a small file instead of striding a large
one — which is where the byte cost of a zoomed-out view lives, and the view that pays it is the
default one: over the million-vertex bench corpus, **245 of 245 tiles and 20,901 kB to draw 15,625
points**, 98.4% of it read and thrown away.

**Both paths select the same rows by the same predicate**, so a corpus without the levels draws the
identical picture and only reads more. That is the property that keeps the pyramid from being a
second contract, and it is held as a test rather than as a sentence:
`crates/fossil-layout/tests/levels.rs` diffs each written level against the predicate evaluated over
the payload, `EXCEPT ALL` in both directions, every column and not only the id, with the row count as
a separate statement because a duplicate and an omission net to zero. Shifting the writer's predicate
by one level fails exactly that test and leaves the others green.

Two things about the pyramid are decided, and each is spelled where it lives —
`crates/fossil-sinks/src/manifest.rs, VertexLevels` and
`crates/fossil-layout/src/layout/pass.rs, write_levels`.

- **Quarters, and the pyramid is complete.** A zoom step doubles the linear scale, which quadruples
  the area and so the points, so a level per quarter is a level per zoom step — in halves one step
  crossed two levels and a writer needed a window constant to bound what it wrote. Every level from
  1 down to the one that fits a single tile is written, and coarser than that buys nothing because
  one tile is already one range request. The series `1/4 + 1/16 + …` is **a third of the type
  whatever V is**, the same third an OME-Zarr pyramid pays, so there is no window and no floor left
  to bound it; the one size that gets no pyramid is the one a single range request already answers.
- **The manifest declares the scales.** A level is one `projections` entry carrying `scale: 4^k`, so
  the list is `log4(V / chunk_size)` entries, nothing here grows with the graph, and a reader
  recomputing it from `vertex_count` would be reimplementing the writer and would 404 the day the
  policy moved. One function chooses them, and both the pass that writes the files and the caller
  that declares them call it. **The exponent never leaves the writer**: what crosses into the
  document is the product, so addressing a level is the payload's own shift plus `log2(scale)`.
</Callout>

<Callout type="warn" title="The edges get a pyramid, and it is not the one this said was empty">
**This callout was titled "the edges get no pyramid" and the measurement under it was the wrong
set.** A level's **induced** edge set — both endpoints in it — nests exactly as its vertices do, so
it is not refused for a centroid's reason; it is refused because it is empty. A level keeps 1/2^*k*
of the vertices and an edge needs two of them, so it keeps on the order of 1/2^2*k* of the edges:
measured over a fixture of **27,976 edges** at mean degree 14, the three written levels induce
**1, 0 and 0**.

The set a camera draws is not that one. `view` keeps an edge with **one** end drawn and both ends
positioned, so the edges a coarse view draws scale with the marks — `V/4^k · degree` — and not with
their square. **Level *k* of a relation is therefore the edges incident to a level-*k* vertex**,
`src_dense % 4^k = 0 OR dst_dense % 4^k = 0`, and each row carries **both endpoints' coordinates**
so that the line can be drawn without opening the payload the pyramid exists to avoid. It is a
decimation like the vertex one: every row a real edge between two real vertices at their real
positions, nothing contracted, and it nests because its vertices do —
`crates/fossil-sinks/src/manifest.rs, Projection` and
`crates/fossil-layout/tests/levels.rs, an_edge_level_holds_the_incident_edges_at_the_payload_s_own_coordinates`.

What this page refuses is unchanged and it is the same refusal twice: an edge between two survivors
standing in for a path through vertices nobody drew is synthetic, and replacing it with the real path
as the camera zooms moves every line on screen. The reader's half is outstanding — see
[the reference viewer](/docs/design/reference-viewer), which carries what a frame costs with and
without it, and until it lands links come from the payload's own `by_source` tiles.
</Callout>

### Which tiles a rectangle touches is the footers' answer, and only theirs

The addressing reads no bytes, so it cannot answer a rectangle: `tilesFor` takes tile numbers, and
the host supplies them from the per-tile `x`/`y` statistics Parquet writes into every footer.
`open` is the layer that has an engine to read them with,
and it buys them while the corpus is opening rather than lazily — a request between the camera
moving and a URL being computable is the `viewport` verb this format deleted.

**A second index over that question was written and deleted.** `vertex/<Type>/codes.json` published
the first and last Morton code of every tile, and it pruned tighter than a footer's box because the
box is the rectangular hull of an arc that snakes. What it cost was a document per corpus, a request
to fetch it, a parser, a guard, a code path, a fallback for corpora written before it, and a field
on the cost report whose entire content was which of the two indexes had answered.
[`design/discarded`](/docs/design/discarded) carries the measurement and what would bring it back.

`apps/corpus/integration/address-cost.test.ts` is what the surviving path costs, in tiles, in `Range`
requests and in kilobytes, against a denominator DuckDB supplies: the tiles that actually hold a
vertex inside the rectangle, so a path answering with fewer has not read less, it has drawn a wrong
picture.
...
- **Its addressing model is this one.** A metadata document plus chunks addressed by a key computed
  from the index, no server, store-agnostic. That is the engine-free depth of `open`, spelled
  differently — and its `multiscales` convention is the level pyramid described above, already
  standardised.
...
title is the reason: none of the six draws, the package's door is `open`, and every type it
...
rename — every `fossil_graph::plan` in the conformance tables, the `fossil_graph_wasm` artefact
...
<Callout type="info">
  The rename touched no arithmetic. What changed the count of readers of
  `conformance/expected.json` was the commit after it, in which the published TypeScript stopped
  being a second implementation of `fossil_graph::plan` and [became a binding over
  it](#conformance-is-a-round-trip-and-what-it-rests-on-is-not-a-headcount).
</Callout>
...
fossil publishes is the address — `open` with no engine lent to it, one reader, no bytes — and
what draws is `@kanzo-tech/graph`, a bounded WebGL canvas over cosmos.gl whose whole data contract
is one method:
...
other source, and it is a *consumer* of [the one door](/docs/design/one-door): a viewport becomes a
finite box, the renderer's `perPixel` becomes the canvas it is drawn into, `frame` answers, and what
is left in the file is the mapping between two vocabularies. Nothing in it knows how cosmos.gl holds
a buffer, and nothing in the renderer knows what a corpus is.
...
- **No second engine, and this is the constraint rather than the preference.** Those bindings are a
  ...
  is what fails.
- **No engine in `@fossil-lang/corpus`.** The renderer needs bytes; the *host* reads them, and the
  package still links none. What it no longer offers is a *WASM-free* way in — the
  `@fossil-lang/corpus/address` subpath was that, and composing a URL now loads
  `fossil-graph-wasm`, which is what the addressing having one implementation costs.
...
**The heading is the whole of it: a corpus does not know.** The fold is the right answer to a
capacity a corpus cannot see, and it stays the answer — what
[a declared channel](/docs/design/position) changes is not the fold but the moment: 128 and 8 are
known on two sides of a boundary and nowhere together, so the picture is the first place anyone
finds out. A domain on the manifest is the corpus saying its own number, which is what lets the two
be compared before a mark is drawn rather than after.
...
graph LR
  ...
  C --> RS["fossil_graph::plan · native, usize 64"]
  C --> WS["the same plan · wasm32, usize 32<br/>re-entered from JS as open"]
  JS --> D{"diff"}
  RS --> D
  WS --> D
...
**Two implementations that never read each other, and three legs of diff over them** — the second is
entered twice, natively and as the wasm32 build the published package binds. The numbers all three
are diffed against come from a **full scan** — every tile read with no addressing at all and
filtered in SQL, a third thing neither implementation wrote. All of them prune and all have to land
where an unpruned read already is, which is what closes GraphAr's gap: an assertion here cannot be
satisfied by agreeing with another implementation, only by agreeing with the bytes.
...
**This said three readers, and there are two.** The reader in Rust exists because **both readers
used to be JavaScript**, and a mistake they share is invisible to a diff of the two — a shift taken
as signed, a count that went through a `Number`, exactly the borders `vectors.json` publishes.
Then the second JavaScript one went: the published TypeScript was a thousand lines that agreed with
the Rust because people kept making it agree, nothing compared the two, and it is a binding over
`fossil_graph::plan` now. So `packages/corpus/tests/conformance.test.ts` is not a second
implementation diffed against the first — it is **the same reader re-entered at wasm32 width**.
`apps/corpus/conformance/reader.mjs` carries the independence.

`fossil_graph::plan` is that reader. It goes through no struct of the writer's — `fossil-graph`'s
...
`apps/corpus/conformance/containers.mjs` and `apps/corpus/integration/containers.test.ts` are that diff:
...
That second file is not there to be read any more, by this consumer or a future one: the writer
stages no Parquet at all, and the tiles are the first and only time the payload is written. Which
does not retire the lesson, because the defect was a reader composing a path the manifest had not
given it.
