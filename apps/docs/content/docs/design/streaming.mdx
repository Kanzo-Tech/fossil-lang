...
| fixture | adjacency rows | the pass | `estimated_peak_bytes` |
| --- | --- | --- | --- |
...
the same line — so attributing Louvain's cost to adjacency rows was mechanically defensible and
unverified. These three hold the vertex file byte-identical and move only the edges. The measured
slope is **6.7 bytes per adjacency row**; a per-vertex attribution predicts a flat line. It is rows.

The constant is 14 rather than 6.7 because it has to bound a *peak* rather than fit a slope, and the
phase that now sets the peak is the last one — `remap adjacencies + write edge tiles`, which holds a
whole orientation as Arrow at once and measures 13.2 / 14.5 / 15.0 B/row at two, four and ten
million and 14.2 at four million and degree twenty-eight. The ten-million row above is the **larger**
of two runs of one build (5.08 and 4.97 GiB), because a bound fitted to the luckier of two runs is a
bound that fails on the unluckier.
...
rewrite the number the phase had been reporting. The phase still bills **+1.95 GiB** at ten million;
here is where it goes (`examples/enrich_memory 10000000 14`, degree 14, 69,937,280 rows per
...
It comes back the day Louvain's high-water is the pass's high-water — a corpus whose level-zero
contraction survives at a much higher ratio than the 79% measured here, or a remap that stops holding
an orientation as Arrow. Both are measurable before anything is written.
</Callout>
