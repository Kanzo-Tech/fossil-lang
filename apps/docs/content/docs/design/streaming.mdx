...
graph LR
  ...
  G --> F["<b>try_for_each_manifest</b><br/><i>seam 2 · the manifests only</i><br/>no payload, so no round trip"]
  G --> E["<b>enrich_layout_within</b><br/><i>the same number, honoured differently</i><br/>takes the batches, estimates, refuses"]
  F --> W[("GraphAr tree on disk<br/>713 MB · 87 files")]
  E --> W
```

**Seam 2 forked, and that is the change.** It used to be the only edge out of `GraphArData`: the
whole corpus was encoded to Parquet, written, and then re-read by the pass, which rewrote it as tiles
and deleted what it had read. The manifests still take that edge; the payload takes the second one,
straight from the same batches. [cost](/docs/design/cost#the-redesign-and-what-is-left-of-the-table)
has the bytes that made the case — 43.3 MB of 115.9 written and then unlinked, on a corpus of 72.6 —
and the digests that say the tree it writes is unchanged.
...
with no disk manager under them, so it **refuses**: `estimated_peak_bytes` over three numbers the
batches already carry — rows, adjacency rows, resident Arrow bytes — checked before a single tile is
gathered. Over budget is an error at second zero that writes nothing at all, instead of an
out-of-memory at second two hundred.

Those three used to be Parquet footer fields, read off the staged files. A footer read is cheap and
that was never the point; what the input being in hand changes is that a refusal now touches no
filesystem, and that the payload term is per resident Arrow byte rather than per uncompressed
Parquet byte — the same product, because the 1.30 between them *was* the decode.
...
`crates/fossil-layout/tests/budget.rs` runs one input corpus twice, once unbounded and once under a
...
rather than overruled: with the contraction's maps gone the peak was 5.08 GiB, and 90 MB against
3.4 GiB recovered is not a trade any more. It is 3.94 with the remap's copies gone too.
</Callout>
...
There is also a floor now, `LAYOUT_BASE_BYTES`, **32 MiB**. Three proportional terms make a small
enough corpus cost nothing, and the writer's row-group buffers are the size they are: a
ten-thousand-vertex corpus holds 6.21 MB against 4.00 MB of linear terms. Without it the guard that
samples the bound was admitting a sixty-thousand-vertex run on 22.8 MB and watching it hold 30.1.

It was 16 MiB, and what re-fitted it is the pass taking Arrow instead of opening Parquet — **not a
change in what the pass holds.** The fixture used to write its corpus to disk and drop the arrays, so
the guard's baseline was taken with those pages freed and the pass reused them; the batches are live
now, as they are in a real run, and `peak − baseline` stopped being biased low. Seven debug runs at
sixty thousand vertices put the constant term at 12.5–16.5 MB alone and **19.9 MB in a loaded
batch**, which is how CI runs it, so the old floor sat below the worst observation and flaked instead
of failing.
...
<Callout type="warn" title="The level-zero pass was worth nothing, and it is worth something again">
The peak is **no longer monotone**, and one page's arithmetic assumed it was. The ten-million run
prints `flatten + order + place  −3.35G` — the level-zero quotient graph being dropped, in pages the
allocator hands back. `estimated_peak_bytes` still *sums* its terms, which is a bound on the maximum
rather than a reading of it, and it is the direction the calibration wants.

**A level-zero pass that streamed the CSR from disk instead of holding it was closed here as worth
nothing, on a measurement, and the measurement has expired.** What closed it was that Louvain's
high-water sat 1.17 GiB below the pass's — 3.55 GiB while it ran against a 4.72 peak reached in the
last phase, after every byte of Louvain's had been given back. The last phase was the adjacency
remap, and it has stopped holding an orientation as Arrow.

At phase granularity now, at ten million: `community_hierarchy` closes at **3.94 GiB and that is the
pass's peak**, and the remap closes at 3.48 and never reaches it. **Louvain's high-water is the
pass's high-water**, which is the exact condition this callout named for the argument coming back.

What it would take off is `read CSR + CSC`'s **+0.78 GiB**, held for all 127 s of Louvain, and it no
longer changes `estimated_peak_bytes` by zero either: `ADJACENCY_ROW_BYTES` is fitted on that CSR at
8 B/row, and the next-largest per-row term is the remap's four. **It would roughly halve the term the
budget charges per adjacency row**, which is what `--memory-gib` admits. What it costs is a Parquet
scan per sweep, and that is the number nobody has: thirty-two sweeps against a CSR that is currently
free to read.

The other route is unchanged and still open — a corpus whose level-zero contraction survives at a
much higher ratio than the 79% measured here. Both are measurable before anything is written.
</Callout>

### One number, two stages: it is **per stage**, and that is a decision

`--memory-gib 4` is spent twice — once by the executor's pool, once by the layout pass — and neither
half knows what the other took, so the write path is bounded at something closer to eight than to
four. That was written down as an open edge. It is decided now, and the decision is the weakest of
the three available: **the number bounds each stage of the write path, not their sum.** Both halves
already implement exactly that; what changes is that it is stated, and that the two alternatives are
refused with reasons rather than left as work.

**Splitting it is refused because the two numbers are not the same kind of number.** The executor's
is a reservation ceiling on a `FairSpillPool` — the tracked operators inside it, which is why a
declared 4 GiB has been measured with the process at 9.87 GiB. The layout's is an arithmetic
estimate of its own `Vec`s. Four split into two and two makes the executor spill more and the layout
refuse more, for a process peak that neither number was ever a bound on, and it gives the flag a
meaning that depends on a ratio nobody can see.

**Sequencing it — the layout told what the executor left resident — is the one that is right in
principle, and it is refused for now on three grounds, in the order that decides it:**

1. **What is resident at the seam is not the executor's transient.** It is `GraphArData`: the corpus
   as a value, 1.64 GiB of Arrow at ten million, and it is held by the *caller* for the whole
   duration of the layout pass — `fossil-cli`'s `run` keeps `graph` alive across
   `enrich_written_layout` because `RunReport::of(&graph)` comes after it. Adding that to the
   layout's bound would bill the layout for somebody else's live value. The fix is to stop holding
   it, not to shrink the second number.
2. **The layout has no reading of "what is resident" that means the same thing in both hosts.** RSS
   is a `ps` call natively and there is no resident set at all on `wasm32`. A budget that consulted
   it would admit different runs in the tab and on the CLI — and two paths through one `LayoutIo`
   disagreeing about what they do is the single thing this design does not permit.
3. **An RSS reading at entry over-counts by construction.** Pages the allocator has not returned are
   pages the pass will reuse rather than add to. At ten million under a declared 4 GiB the pass
   *enters* at 6.82 GiB, so a check of that shape would refuse every bounded run at this size,
   including the ones that then fit.

**What brings sequencing back is a value dropped, not an instrument added.** The day the caller
builds its report before the pass rather than after, what crosses the seam stops being a resident
set to sample and becomes a number to be handed over — `GraphArData::arrow_gib` is already that
number — and `declared − carried` is arithmetic. Until then the honest sentence is the one the pass
now prints when it refuses: **the number is this stage's.**
...
`crates/fossil-df/src/lib.rs, finalize_vertex` collects each vertex projection before registering it as a
...
`try_for_each_file` built each file, handed it over and dropped it before the next; `to_files()` was
that with a `Vec` on the end, for the browser host, which genuinely needs a list. **Peak RSS at
ten million moved from 10.60 GB to 10.58 GB.** Two identical runs differ by more than that, so the
number is not small: it is absent. The +0.58 GiB the probe billed to the encode phase was the
encoder's working memory — the output was 713 MB across six files, so holding six of them was never
that figure.

The change stayed, and the reason has to be stated separately from the number, because mixing them is
how a refuted hypothesis quietly survives: holding N files in order to write them one at a time had
**no reader on that path**. Indefensible per file, and saying so is not saying it saved megabytes.

**And the question stopped being asked**, which is the right kind of ending for it. The encoder does
not encode the payload any more — `crates/fossil-df/src/files.rs, try_for_each_manifest` is what is
left of it, and the layout pass writes the tiles from the batches directly — so one-at-a-time against
all-at-once is now a question about a handful of YAML documents. What the measurement bought was
knowing that this seam was **not** where the seventeen gigabytes were, which is what sent the search
to the one that mattered: not how the staged bytes were held, but that they were written at all.
[cost](/docs/design/cost#the-redesign-and-what-is-left-of-the-table) has that one.

### 3. The layout read the whole edge list — landed 2026-08-05

`enrich_layout` walks both orientations **as the CSR they already are**, one `RecordBatch` at a time,
the two columns arriving as the `u32` slices `CsrBuilder::push` wants; no row is ever a Rust tuple.
Gone with it: a `Vec<(u32, u32)>` of every edge (568 MB at ten million), a degree-counting pre-pass,
and an 80 MB cloned cursor scattering random writes over the 568.

It read those batches out of Parquet at the time, and takes them from the executor now — which
removed the decode but not the reason this entry is here. The shape it argues for is that the pass
consumes an orientation in the order it already has, and that shape is what made the input being
Arrow a substitution rather than a rewrite.
...
written — `by_source` has zero out-of-order rows by `src_dense` across 71,024,690, and
