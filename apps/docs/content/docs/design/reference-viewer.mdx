...
`apps/corpus/integration/frame-levels.test.ts` holds it against a fixture corpus with a pyramid and
...
**And it measures the decimation only.** Every figure in the last section was taken off the corpus's
positions rather than through the door, because no corpus writes a summary and the door has nothing
to open. That is not a defect in the instrument and it is not a gap in the writer: when something
writes cells, they are measured the same way — one rectangle, both artefacts, through the door — or
the number is again a property of whatever read them.
...
**This paragraph used to end "nothing has asked for it yet". It has been asked for, and it has been
measured**, and the four sections below are what the measurement says. None of it reverses the
model; what it does is move the level files from *the* answer at a far zoom to *an* answer, and
leave one question about them open in its correct form.

### A level and a cell are the same node, answered two ways

Level *k* is the rows whose `dense_id` is a multiple of `4^k`, which over a Morton-ordered
`dense_id` is one real vertex per quadtree cell of depth *k*
(`crates/fossil-layout/src/layout/pass.rs, write_levels`). The cell is therefore already in the
model, and has been since the numbering was chosen. A level names a cell and returns a **sample** of
its rows; a summary names the same cell and returns its **aggregate**. Same node, two answers, and
the arithmetic a reader already composes reaches both.

That is what keeps the third artefact from being a rival hierarchy. It is not another way to build a
tree over the graph — the tree is the one the positions already induce — it is the other question
one can ask of a node in it. Which is also why the summary does not inherit
`crates/fossil-sinks/src/manifest.rs, VertexLevels`' block: a level declares a `scale` a reader
spends as arithmetic, and a summary declares a count, for the reasons
`crates/fossil-sinks/src/manifest.rs, CellTree` is a block of its own and not a `Projection`.
[Cells](/docs/design/cells) is the page that rules on that, and the grouping it rules over is
Louvain's rather than the plane's; what is measured below is the plane's.

### The level files are a cache, and the question is whether a reader still wants the sample

Invariant 3 above already says what a level file is: `dense_id % 4^k == 0` **is** the level, and a
written `l{k}/` is an optimisation of it. Stated as cost rather than as contract, that is: **giving
one real vertex in `4^k` is reading the payload with a stride and throwing away `4^k - 1` rows out
of every `4^k`, and the file only avoids those bytes.** Nothing else is bought. The picture is
identical either way — that is the property, not a side effect.

So the question the measurement opens is **not** "levels or rungs", which would be a question about
two hierarchies and there is only one. It is: **is there a reader left that wants a decimated view
of REAL rows once the cells can answer?** If there is not, the level files are dead weight — bytes
the writer pays, and a second set of artefacts to keep nesting, for a worse picture than the
summary gives at the same zoom. If there is, they stay exactly what invariant 3 says they are: a
declared optimisation of a predicate, and not an artefact carrying a meaning of its own.

What this page proposed would settle it is a reader that wants to **click something real at a far
zoom** — a sample row is a subject, a cell is not, and a picture nobody interrogates does not need
the sample.

### It is settled, and the test this page proposed is the half that failed

**Nobody clicks.** There is no click handler, no hover, no tooltip, no bookmark and no deep link
anywhere on the coarse path in this repository. `Frame` carries `denseIds`, positions, categories
and links, and no `subject` — `packages/draw/src/encoding.ts` says so with a measurement beside
it, `Slice.subjects` costing 1.87× the tile, and `apps/playground/src/crossfilter.ts` excludes the
column from its view for the same reason. The identity verbs take an IRI *in* and know nothing about
levels. This page already conceded the other half of it above: there is no level of detail for the
identity index.

**And the sample stays anyway, because identification at a coarse zoom happens by *address*, not by
name.** Three readers do it, and none of them would get slower without a level — each would stop
working:

| what it does | why a cell cannot serve it |
| --- | --- |
| `apps/playground/src/crossfilter.ts` paints a `Uint8Array` mask indexed by `dense_id`, on every brush, at whatever level the camera settled at | a cell has no `dense_id`, so there is no index entry to paint — the mask is not wrong, it is impossible |
| `packages/draw/src/residency.ts` pools answers taken at *different* levels into one interim picture and dedupes them by `dense_id` | coherent only because level `k+1`'s rows are a **subset** of level `k`'s. Rung `k+1`'s cells are a disjoint space from rung `k`'s, with no containment a set can see |
| `packages/corpus/src/corpus.ts` exempts `pinned` from the level and pays a second payload read to serve it | there is nothing to exempt over an artefact whose rows are not addressable |

So a coarse mark has to **be** a row, even though nothing in this tree ever names one.

### Two more reasons, and one of them is not about the reader at all

**An edge level is the only place an edge knows where its ends are.** Its rows carry `src_x`,
`src_y`, `dst_x`, `dst_y`. The adjacency carries two `dense_id`s and nothing else; a rung's quotient
carries two cell ids and a weight. So an edge level is self-drawing from one file, and neither of
the others is — and the quotient is not a substitute in kind: its edges are synthetic, which is the
one substitution [what was discarded](/docs/design/discarded) refuses outright, because replacing a
line with the path it stands for moves every line on screen.

**Nothing reads a rung.** `crates/fossil-graph` — the read surface the format exists for — has no
verb, no plan and no type that names a rung, a cell or a quotient, and `packages/corpus` says it of
itself in `packages/corpus/src/corpus.ts`: *nothing on this side writes or reads a rung*. No guard
opens one; the conformance corpus declares `rungs: []`. The cell pyramid today is a writer and a
manifest block. Retiring the levels would therefore not move a capability from one artefact to the
other. **It would leave the coarse read path with no implementation on either side of the wasm
boundary.**

### The bytes say the opposite, and saying so is the point

The economic half of the argument goes the other way, and it is worth stating against the decision
rather than around it. Measured on com-DBLP by
`crates/fossil-layout/tests/level_vs_rung.rs, what_a_level_costs_and_what_a_rung_costs_on_com_dblp`,
at the fifteen thousand marks a megapixel canvas asks for — where the pairing is level 2 against
rung 1, both at 19,818 marks:

| | level 2 | rung 1 |
| --- | --- | --- |
| bytes a coarse view reads | 2,410 kB | **1,336 kB** |
| lines drawn past the three-pixel floor | 136,910 | **272,732** |
| bytes per line drawn | 18.0 | **5.0** |
| share of the type represented | 6.25% | **100%** |
| the whole pyramid, against payload + adjacency | 83.1% | **8.1%** |

The level's defining ratio survives — it is 5.07× cheaper than striding the payload at that scale
and 57× at its coarsest — but the rung beats it on that same ratio at the same mark count, 9.1×. So
**the ratio surviving is no longer an argument that the artefact is needed**, and the levels are not
kept because they are cheap.

<Callout type="warn" title="And this table flips on a generated fixture, which is why it is not the reason">
On the planted fixture the level wins instead, by 1.24× to 3.46×. That corpus's ten per cent of
uniform-random edges triples its finest quotient — 871,261 rows against com-DBLP's 274,647 — so
whoever weighs this on generated data reaches the opposite conclusion from whoever weighs it on a
real graph. **One more real graph of a different shape is what would settle the byte question**, and
it is not settled here. What is settled is the one above it, which no byte count can move.
</Callout>

### So the question closes, and not as «kept as a declared optimisation»

The two artefacts are not substitutes with a winner. They are complements, and the crossover is
measured:

- **Below 1,239 marks only a rung answers, and it answers well.** The level pyramid stops at its
  one-tile floor, which is six of the nine scales on com-DBLP, and the rungs covering those six
  scales are rungs 4 through 9. The same instrument that weighs the bytes also runs each rung through
  [the camera's fidelity measure](/docs/design/camera#1-fidelity-a-coarse-view-is-the-same-image-with-fewer-points),
  on a grid fitted to each rung's own mark count, and **every one of them clears its tolerance** —
  rung 4 at 0.2230 total variation against a null of 0.3700, and the margin widening the coarser it
  gets, to +0.3341 at rung 7.
- **Above 79,270 marks only a level answers**, because a base of sixteen puts the finest rung below
  it.
- **In between they both answer, to different questions** — 6.25% of the vertices with real
  identity, or 100% of them summarised. A reader choosing between those is not choosing a cheaper
  encoding of one answer.

<Callout type="warn">
**The first bullet was narrowed to «and nothing answers well», on a reading that the six rungs below
1,239 marks do not clear the camera's tolerance. That narrowing is withdrawn.** It came from measuring
310 marks against a fixed 48×48 grid — **7.43 bins per mark**, where the rung and its null both pin
against TV 1 and the −0.0016 between them is binning noise. Re-measured at about `sqrt(marks)` bins
per axis by `crates/fossil-layout/tests/level_vs_rung.rs, fidelity`, rung 4 beats its null by
**+0.1470** and rung 7 by **+0.3341**, a margin that grows rather than decays down the pyramid.
[A fidelity measure has a resolution](/docs/design/camera#a-fidelity-measure-has-a-resolution-and-below-one-bin-per-mark-it-is-measuring-itself)
is the general rule the mistake violated, and
[the quotient saturates down the pyramid](/docs/design/cells#the-quotient-saturates-down-the-pyramid-and-the-marks-do-not)
carries the full retraction.

**What this bullet can honestly claim is the complement, and the reason is not fidelity at all:** a
rung is the only artefact below 1,239 marks, it carries a real picture there — and **nothing in this
repository ever asks for one.** A camera spends a budget in marks and the smallest budget anything
here spends is 15,000; `apps/playground/src/tiles.ts` spends 20,000. Only rung 1, at 19,818, is above
that. So the cell pyramid's depth is not capped by what it can draw; it is uncapped and unvisited.
Whether the writer should keep emitting rungs nothing asks for is
[open](/docs/design/cells#what-this-does-not-settle).
</Callout>

Invariant 3 is unchanged and now carries its reason: a written `l{k}/` is an optimisation of
`dense_id % 4^k == 0`, and what makes the optimisation worth writing is not its bytes but that its
rows are addressable.

### What the summary costs, measured

Over com-DBLP — 317,080 vertices, 1,049,866 undirected edges — with a quaternary over the positions:

| | |
| --- | --- |
| fusion per level, below saturation | **1.0–1.8×** |
| fusion per level, from level 9 | **exactly 4.00×** |
| occupancy at level 8 | **61,210 cells of 65,536 — 93.4%** |
| the whole pyramid, every level | **1,375,552 cells — 4.34× V** |
| the pyramid from the saturation scale | 61,210 × 4/3 = **81,613** |

Two things in that table. **Summarising every cell at every level costs more than the graph** —
4.34 times the vertex count, because below saturation a coarser level merges almost nothing and each
level is very nearly a copy of the one under it. And **from the saturation scale up it is `1/3` of
the base, to the digit**, which is invariant 4's fraction arriving unchanged at an artefact that is
not a decimation: the geometry is the same geometry.

Which makes the base a **declared parameter and not a property of the corpus**:

| vertices per base cell | cells, as a share of *V* |
| --- | --- |
| 5 | 25.7% |
| 21 | 6.4% |
| 83 | 1.6% |
...
Nothing in the graph chooses that row. **The resolution of the screen does** — a base cell is worth
writing when it is about the size of a mark, and every figure above moves by an order of magnitude
across four plausible answers to that. It is the same shape of decision as a branching factor being
declared rather than inherited, and it is the reason a summary block would have to carry the number
rather than derive one.

### What the summary cannot do, and it is the edges

The quotient of the spatial partition — the edges between cells — is not drawable at any coarse
scale, measured on the same corpus:

| cells | quotient |
| --- | --- |
| 15,303 | mean degree **20.1** |
| 3,826 | mean degree **57.8** |
| 957 | mean degree **147.2** |
| 240 | **79.8% density** |

At 240 cells four in five of the possible pairs have an edge, which is a filled square and not a
picture. And it is **denser than the community quotient at comparable counts**, which stays around
23 on the same graph — [cells](/docs/design/cells) has that table. Grouping by position is worse
for edges than grouping by community, and grouping by community is already too dense to lay out.
**No coarse quotient of a real graph is drawable**, and no choice of grouping rescues it.

So the third artefact summarises **the field and not the relation**. It answers *where the vertices
are*, which is the question density was raised for, and it answers nothing about who is connected to
whom. The edges keep being drawn where they are drawable: at the bottom, where the quotient is the
graph — mean degree 7 at density 0.01% — off the relation's own level set, which is the second
artefact and is unaffected by any of this.

One caveat on reading the field at all. A heat map of `derived` positions is a picture of the
layout's own regularities rather than of the world — `crates/fossil-sinks/src/manifest.rs, Provenance`
is where a corpus says which it has, and [where x and y come from](/docs/design/position) is the
argument. The summary is worth writing for a `geographic` or an `embedded` plane in a way it is not
for a plane a drawing algorithm chose.
