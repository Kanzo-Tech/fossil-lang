...
`crates/fossil-sinks/src/manifest.rs, HolonTree` is a block of its own and not a `Projection`.
[Holons](/docs/design/holons) is the page that rules on that, and the grouping it rules over is
...
and links, and no `subject` — `apps/playground/src/encoding.ts` says so with a measurement beside
...
| `apps/playground/src/residency.ts` pools answers taken at *different* levels into one interim picture and dedupes them by `dense_id` | coherent only because level `k+1`'s rows are a **subset** of level `k`'s. Rung `k+1`'s cells are a disjoint space from rung `k`'s, with no containment a set can see |
...
**Nothing reads a rung.** `crates/fossil-graph` — the read surface the format exists for — contains
no occurrence of `holon`, `rung`, `cell` or `quotient`; neither does `packages/corpus`; no guard
...
[the quotient saturates down the pyramid](/docs/design/holons#the-quotient-saturates-down-the-pyramid-and-the-marks-do-not)
...
[open](/docs/design/holons#what-this-does-not-settle).
</Callout>
...
23 on the same graph — [holons](/docs/design/holons) has that table. Grouping by position is worse
