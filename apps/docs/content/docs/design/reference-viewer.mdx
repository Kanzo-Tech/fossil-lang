...
direction:
  summary: A frame reads the coarsest artefact that can answer it and reports every byte it opened — the door reads both pyramids now, and what is left is the ledger, which weighs the two halves of a frame by rules that are not the same one.
---
...
the list of places where the tree does not meet it — including the ledger that tells a caller what a
frame cost, which is the one invariant of the five that does not hold.
...
tab. `ViewCost` reports `tiles`, `runs`, `bytes` and `read` — and `bytes` counts whole vertex tiles
and no adjacency at all. **This invariant is false in both directions at once**, which
...
graph LR
  ...
  L --> E["edge/Person_knows_Person/l6/<br/><b>1,332 kB</b> · written, and read"]
...
| **d · + edge levels** | 201 + 1,332 = **1,533 kB** | reported, by gap 2's arithmetic | 15,625 · 60,136 · 62,024 |
...
for links at a level both pyramids are written at. The edge level at *k*=6 holds
...
Five, in the order they were written. The first is the one that closed; what is left of it is stated
