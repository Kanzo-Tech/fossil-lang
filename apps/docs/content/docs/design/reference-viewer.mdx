...
- **Below 1,239 marks only a rung answers.** The level pyramid stops at its one-tile floor, which is
  six of the nine scales on com-DBLP — and a corpus with no cell pyramid has no coarse view there at
  all, where a corpus with no level pyramid has a slower one.
