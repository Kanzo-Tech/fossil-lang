...
### The level files are a cache, and the open question is whether a reader still wants the sample
...
What would settle it is a reader that wants to **click something real at a far zoom**. A sample row
is a subject — it has an identity, a tile, an entry in the index, and a rectangle around it refines
into its neighbours. A cell has none of those: it is not a subject and
[identity](/docs/design/identity) is the page that says why nothing derived gets to be one. A
picture nobody interrogates does not need the sample; a picture somebody clicks does.
