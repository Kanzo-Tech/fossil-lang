...
The app still reads none of the four, because it asks for links at every zoom and a level answers
only a view that does not — which is gap 4, and is untouched by any of this.
...
`crates/fossil-layout/src/layout/pass.rs, write_edge_levels` emits the files, `apps/corpus/guards/fixture.mjs`
...
type, so `Corpus.frame` has no address to compose and no way to open one. The consequence is exact:
