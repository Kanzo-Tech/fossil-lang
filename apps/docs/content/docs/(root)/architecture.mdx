...
graph LR
  ...
  C["<b>corpus</b> · 7 crates<br/>graph · graph-wasm · mcp · layout<br/>sinks · resolver · policy"]
  X["<b>no home</b> · 3 crates<br/>mem-probe · kanon · xtask"]
...
**54 cross-group edges**, and one pair still runs both ways — on a single edge. Every number above
...
| engine → corpus | `fossil-cli → fossil-sinks` | the same constant, read by the binary that prints the manifest |
...
| engine → corpus | `fossil-engine → fossil-policy` | reading the document is a host's job, the way reading a shape document is |
| engine → corpus | `fossil-cli → fossil-policy` | `--policy` is a path, and a path becomes a `PrivacyPolicy` before the host is called |
...
`fossil-kanon` is filed under *no home* for a stronger reason than the other two: it has no
dependents **and** no dependencies inside the tree, so it is the one node this diagram draws with no
arrow at either end. It is a disclosure-control pass over Arrow arrays that nothing in the pipeline
calls yet, and filing it beside a crate it does not touch would be filing it by hope.
[Deriving k-anonymity](/docs/design/anonymity) is the argument for it and states what would give it
a caller.

The `@conn` locator has left: it is `fossil-locator`, a crate with no dependencies at all, and the
edges it cost are why the total moved. Filing it under `language` is the same kind of move
`fossil-sinks` and `fossil-resolver` got — by subject, and it changed no code — except that this one
did move a file. What has **not** happened is the rest of what is below this diagram: `fossil-base`
still holds a `tracked` query, and `fossil-engine` is still a sack.
...
The file abstraction is the part that has gone: `fossil-locator` is the rule that says where a
written path points, which is the one thing in `fossil-base` that decided something about a program.
`base-db` keeps its VFS in another crate for that reason and this is the same move.
...
| a new leaf holding `DescriptorCache` + `InferredDescriptor` | it works, and it is the only one that does: a 27th crate, an import path changed in nine crates, and **nothing removed from `fossil-base`** |
...
**Twenty-six crates over roughly 47,000 lines of Rust** — non-blank, non-comment, measured
2026-08-25, and the twenty-sixth moved lines rather than adding them. Gluon has ~11 crates for 90k, Gleam 15 (six real) for 228k, rust-analyzer 36 for 571k.
