...
**51 cross-group edges**, and one pair still runs both ways — on a single edge. Every number above
...
| engine → corpus | `fossil-cli → fossil-policy` | reading the document is a host's job, the way reading a shape document is: `--policy` is a path, and a `PrivacyPolicy` by the time `host::run` is called |
...
since, so the only honest options were to re-measure or to drop it. **At this size the honest target is five to eight.**
...
The count went 25 → 26 → 25 in one sitting and the line count did not move: `fossil-locator`
arrived by moving a module down, `fossil-engine` left by moving three into the crate that already
used them. Which is the point of the heading: neither number is the verdict.
