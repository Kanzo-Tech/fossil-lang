...
direction:
  summary: The tree is cut by what a piece of code is allowed to know rather than by where it runs, which collapses the deployment targets into shells thin enough to be uninteresting and takes twenty-six crates towards five to eight.
  arguedIn: /docs/design/three-hosts
today:
  summary: Twenty-six crates, forty-six cross-group edges across five subject groups, and one pair of groups still pointing at each other — the engine and the corpus, over four named edges rather than a misfiled crate; `fossil-graph` reaches the rest of the tree exactly once, through its only fossil dependency, `fossil-sinks`.
...
graph LR
  ...
  X["<b>no home</b> · 3 crates<br/>run-status · mem-probe · xtask"]
...
Twenty-six crates, forty-six cross-group edges, and **one pair still runs both ways**. Measured
...
And that relocates one crate rather than renaming it. `fossil-run-status` holds three unrelated wire
contracts — the output of `run`, of `providers`, and of `refs`. They are not a crate: **they are the
outbound half of a protocol**, and in rust-analyzer they would live in the binary. Six crates depend on it and it depends on nothing, which is not being foundational — it is
