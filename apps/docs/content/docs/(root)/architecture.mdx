...
**56 cross-group edges**, and one pair still runs both ways — on a single edge. Every number above
...
| --- | --- | --- |
| corpus → engine | `fossil-layout → fossil-df` | one type, `files::TileWriter`, the row-group container the corpus's payload is written through — moving it would put `arrow` + `parquet` into `fossil-graph-wasm`'s browser bundle, which has neither |
