...
| --- | --- |
| `fossil-ide` | not an IDE: the set of LSP functions, and it exists only because `fossil-lsp` does not compile to wasm32. It also returns `lsp_types` structs directly (`crates/fossil-ide/src/completion.rs:58`), which is the invariant rust-analyzer defends hardest and the one Gleam broke |
