...
| corpus → engine | `fossil-graph → fossil-sinks` | the manifest model, which is the format, filed under the engine by history rather than by subject |
| corpus → engine | `fossil-mcp → fossil-resolver` | `@conn` rendering, a host concern wearing an engine label |

So there are two cores, stitched together in the wrong place — but the stitch is three named
functions and not a crate boundary, and [one engine](/docs/design/one-engine) is where the filing
gets settled.
...
is already a [discarded alternative](/docs/design/discarded) — so the seam between the two cores is
made of the one thing the design has ruled out.
