...
direction:
  summary: The tree is cut by what a piece of code is allowed to know rather than by where it runs, which collapses the deployment targets into shells thin enough to be uninteresting and takes twenty-five crates towards five to eight.
  arguedIn: /docs/design/three-hosts
today:
  summary: Twenty-five crates, forty-five cross-group edges across five subject groups, and one pair of groups still pointing at each other — the engine and the corpus, over four named edges rather than a misfiled crate; `fossil-graph` reaches the rest of the tree exactly once, through its only fossil dependency, `fossil-sinks`.
...
**It is gone** (`0d04c27`). Each contract went back to whoever produces it: `RunStatus` to `fossil-df`,
`ProviderInfo` and `SourceRefInfo` to `fossil-lineage`, which had been importing its own output types
back from it. None became a method of the LSP — `run` is not an editor operation, `providers` is a
...
[six verbs](/docs/characteristics/verbs). The crate that writes is `fossil-df`; the crate that models
