...
[Holons](/docs/design/holons) is the argument for the cell pyramid being a third artefact.
...
| --- | --- |
| `HolonTree::vertices_per_cell` | the base of the pyramid is a choice about what a screen can draw, and a set of points is never full, so no writer derives it from the data |
...
`crates/fossil-sinks/src/manifest.rs, HolonTree` carries no `chunk_size` of its own, because a rung's
...
([holons](/docs/design/holons), [the reference viewer](/docs/design/reference-viewer)).
...
[Holons](/docs/design/holons) carries that argument and this page does not repeat it.
