...
| --- | --- | --- |
| corpus → engine | `fossil-layout → fossil-df` | one module, `files` — the single Arrow→Parquet writer, `TileWriter` for the pass itself. Moving it would put `arrow` + `parquet` into `fossil-graph-wasm`'s browser bundle, which has neither |
...
`fossil-sinks` is: the declared bound is part of what the artefact says about itself. `fossil-policy`
is the crate with no fossil dependency at all — `fossil-sinks` has one, and it is this. Four of the eight `engine → corpus` edges are
...
because `lsp-server` uses crossbeam and stdio, and `crates/fossil-resolver/src/lib.rs:49`, because
...
| --- | --- |
| `fossil-ide` | not an IDE: the set of LSP functions, and it exists only because `fossil-lsp` does not compile to wasm32. It also returns `lsp_types` structs directly (`crates/fossil-ide/src/completion.rs:63`), which is the invariant rust-analyzer defends hardest and the one Gleam broke |
