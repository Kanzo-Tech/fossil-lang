...
- **The memory probe.** `FOSSIL_MEM_PROBE` shells out to `ps` and reads `Instant::now()` — I/O
  ...
  `fossil-mem-probe` now, a leaf with no dependencies. That one `use` was **`fossil-runtime`'s only
  edge to `fossil-base`**, so a native execution runtime no longer links the compiler substrate in
  order to time a phase: `cargo tree -p fossil-runtime -e normal` reaches `fossil-base` on no path.
