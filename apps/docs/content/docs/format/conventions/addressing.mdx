...
## The codes are the anchor

Arithmetic says **which tiles exist**. Which tiles **intersect the window** is a second question, and
`chunk_size` does not answer it: `dense_id` is a vertex's *rank* by Morton code and not its code, and
where a code falls in a ranking is a function of how the positions are distributed. Assuming the
ranks are uniform in the code space is not a conservative guess — over nine windows of a
million-vertex corpus it misses **129 of the 226** tiles that hold a matching vertex, which is a
wrong picture rather than a slow one.

So a vertex type publishes the codes its tiles begin and end on, in a document beside them:

```yaml
# vertex/Person/codes.json              the anchor — two u32 per tile, and the box they were cut on
codes:
  path: codes.json
```
...
```json
{
  ...
  "chunk_size": 4096,
  "tiles": 245,
  "extent": { "xlo": -49.435, "ylo": -46.09, "xhi": 342.257, "yhi": 337.975 },
  "lo": [22025280, 925545790, "…"],
  "hi": [908871311, 1803986609, "…"]
}
```

`lo[k]` is the Morton code of tile *k*'s first row and `hi[k]` the code of its last. Both arrays are
non-decreasing, because the rows are in rank order and rank order **is** code order — which is what
makes two binary searches over them a decomposition rather than a scan. `extent` is the box the
positions were [quantised against](#the-arithmetic), the half a reader cannot derive
without opening every tile; `morton_bits` is the grid, declared rather than left to be inferred by a
reader that has no way to notice the day it changes.

**A path on the manifest and not the numbers in it.** There are two per tile, so inlining them would
make a manifest grow with the corpus — and a manifest is read in full by everything that touches
one, including a reader that wants a row count. The `codes:` block has the shape
[`index:`](/docs/format/conventions/identity#searching-for-one-is-a-second-table-not-a-second-column)
has: the manifest names where, and the file is what a reader fetches when it has a rectangle.

**JSON and not a packed array.** The 245 pairs of a million-vertex corpus pack to 1,960 B and print
to 5,498 B, and both are two orders of magnitude under the 226 kB of Parquet footer that answers the
same question over the same corpus. What the difference buys is a document with no endianness, no
width and no offset table to agree on — legible to anything that can read a number, which is the
criterion the rest of this page is written to.

It is **optional**, on the same argument `index:` is: a corpus without it leaves the window question
answerable and slower, out of the footer boxes below, and a corpus whose payload carries no `x`/`y`
has no codes to anchor. What it does not leave answerable is the window question *for a reader that
has no Parquet reader*, which is the reader the anchor exists for — a camera addressing a rectangle
with `JSON.parse` and arithmetic, no engine and no fetch beyond the bytes it already holds.

Guard: `code-anchor`, and it is not "the document parses". Every `lo[k]`/`hi[k]` is recomputed from
the corpus's own first and last row of tile *k*, quantised in binary32 against the extent the anchor
publishes, and that extent is checked against the corpus's actual bounding box. A plausible anchor
computed from something else draws a plausible picture of the wrong vertices, and no count notices.

## The footer is the index

A code range prunes a rectangle *tighter* than an `x`/`y` box does — the box is the rectangular hull
of an arc that snakes, and the arc is where the vertices are — so what the footer is needed for is
**byte offsets**: under `container: rowgroups` a run of tiles is a byte interval only a footer can
name, and a corpus that publishes no anchor answers the window question here instead. The boxes live
in the footer, and one request buys all of them.
...
**The level-of-detail pyramid is not that tree either**, and it is not the aggregation this
paragraph used to describe. It said a level tile holds *super-nodes that do not exist at level 0*,
each level carrying its own `dense_id` space and its own Morton renumbering. What was built is a
**decimation**, for the reason the design page gives at length: a synthetic centroid cannot nest, so
replacing it with its children moves every point on screen.

**Level `k` is the vertices whose `dense_id` is a multiple of `4^k`.** Over a Morton-ordered
`dense_id` that is one vertex per quadtree cell of depth `k`. Quarters and not halves because that
is what a camera does: a zoom step doubles the linear scale, quadruples the area, and so quadruples
the points, which makes one level exactly one zoom step. Three things follow from the definition
alone, and none of them needs a byte on disk:

- **It nests, by construction.** `dense_id % 4^(k+1) == 0` is a strict subset of
  `dense_id % 4^k == 0`, so refining only ever ADDS and a vertex drawn once stays drawn at the same
  position.
- **Every row is a real vertex** at the position the payload gives it. There is no synthetic node
  anywhere in the pyramid, and `cluster_id` is a colour rather than a level of anything.
- **There is no second `dense_id` space.** A level shares the payload's numbering, so a level tile is
  addressed by the payload's own shift plus `2k` — level `k` keeps one id in `4^k`, so its tile of
  `chunk_size` rows spans that many times the ids: tile `j` covers
  `[j · chunk_size · 4^k, (j+1) · chunk_size · 4^k)`.

So it is **`k` flat tilings of one id space**, not a tree of `k` levels and not `k` id spaces.

### `levels:` — which of them are written

A level is a predicate, so a reader answers any level with or without a file: striding the payload
selects the same rows and reads more bytes. A written level is a **cache of that predicate**, and
what it changes is a byte count.
...
Level `k`'s tiles live under `<prefix of the type><stem>{k}/`, and inside it the corpus's
[container](#two-containers-one-address) spells the filenames exactly as it does for the
payload — `l6/chunk0.parquet` where the payload is `chunk0.parquet`, `l6/tiles.parquet` where it is
`tiles.parquet`. No level writes an anchor of its own: whichever artefact addressed the payload has
already addressed the level.

**The numbers are declared, and that is a departure from the tile-code anchor beside it.** The
anchor is two integers per tile and therefore grows with the corpus, so inlining it would charge
every reader of a manifest for the size of the graph; a level list is at most three integers
**whatever the corpus is**. And it is not derivable: which levels a writer chose to spend bytes on is
a *policy*, and a reader re-deriving it from `vertex_count` and `chunk_size` would be reimplementing
the writer's plan and would 404 the day the plan moved.

`chunk_size` is declared for the same reason rather than inherited from the payload's: a reader
turning a level tile back into a `dense_id` range multiplies by it, and a number that has to be
assumed is one a writer can change in silence.

**Absent is a corpus and not a gap.** A type that fits a single tile declares no `levels:` and draws
the identical picture — one range request already answers it, so there is no coarser view left to
write. Every type over that size carries the pyramid, complete.

Which levels a writer writes is a **policy** rather than something a reader derives, and the policy
is one function: `crates/fossil-sinks/src/manifest.rs, VertexLevels`. `a-level-is-the-predicate`
checks the files against the predicate rather than against the plan, because a corpus is free to
declare a different set as long as what it wrote is what it says.
...
So the composition ships as code, in `@fossil-lang/corpus`, sharing nothing with the verbs but the
manifest:

```ts
const corpus = resolveCorpus({ manifestFiles, base: '/bench/1000000' });
const { vertexUrls, edgeUrls, complete, gaps } = corpus.tilesFor({ tiles, directions: ['src'] });
```
