...
| the edges drawn in it | `by_source/chunk{k}.parquet`, the same `k` — every drawable edge has its source on screen |
| the out-edges of `v` | `by_source/chunk{v >> shift}.parquet`, filtered to `src_dense = v` |
| the in-edges of `v` | `by_target/chunk{v >> shift}.parquet`, filtered to `dst_dense = v` |
...
*The corpus does not publish it* is a property of the artefact: an edge type with no projection
`aligned_by` that direction, or one declaring it with no `path`, has tiles nobody can address, and the
...
Arithmetic says **which tiles exist**. Which tiles **intersect the window** is a second question,
and `chunk_size` does not answer it: `dense_id` is a vertex's *rank* by Morton code and not its
code, and where a code falls in a ranking is a function of how the positions are distributed. What
answers it is the per-tile `x`/`y` bounding boxes Parquet writes into the footer, and one request
buys all of them. Under `container: rowgroups` that same footer is also what turns a run of tiles
into a byte interval.
...
- **There is no second `dense_id` space.** A level shares the payload's numbering, so a level tile is
  addressed by the payload's own shift plus `log2(scale)`, and tile `j` covers
  `[j · chunk_size · scale, (j+1) · chunk_size · scale)`.

So it is **`k` flat tilings of one id space**, not a tree of `k` levels and not `k` id spaces.

### `projections:` — the payload and the levels in one list

A corpus is an order, a cut, and **projections of that sequence** — one block, finest first, in
which the payload is the entry whose `scale` is `1` rather than a special case:

```yaml
projections:
- { path: '',  scale: 1,  file_type: parquet, properties: [ ... ] }
- { path: l1/, scale: 4,  file_type: parquet, properties: [ ... ] }
- { path: l2/, scale: 16, file_type: parquet, properties: [ ... ] }
```

`path` is where the tiles are, relative to the type's own `prefix`, and inside it the corpus's
[container](#two-containers-one-address) spells the filenames exactly as it does for the payload —
`l1/chunk0.parquet` where the payload is `chunk0.parquet`, `l1/tiles.parquet` where it is
`tiles.parquet`. **`scale` is how many `dense_id`s one row stands for**, so a projection holds
`vertex_count.div_ceil(scale)` rows and a reader adds `log2(scale)` to the type's own shift and needs
no exponent of its own. Both are [OME-NGFF's own spellings](/docs/design/prior-art). No projection
declares a `chunk_size` — the cut does not change with the scale — so the type's own number addresses
every one of them, and [the identity index](/docs/format/conventions/identity), which is not a
projection, is the one artefact carrying its own.

**The scales are declared, and they are not derivable.** The list is
`log4(vertex_count / chunk_size)` entries, so nothing here grows with the graph. And which levels a
writer chose to spend bytes on is a *policy*: a reader re-deriving it from `vertex_count` and
`chunk_size` would be reimplementing the writer's plan and would 404 the day the plan moved. One
function chooses them: `crates/fossil-sinks/src/manifest.rs, VertexLevels`.
`a-level-is-the-predicate` checks the files against the predicate rather than against the plan,
because a corpus is free to declare a different set as long as what it wrote is what it says.

**A single projection is a corpus and not a gap.** A type that fits a single tile declares the
payload alone and draws the identical picture — one range request already answers it, so there is no
coarser view left to write. Every type over that size carries the pyramid, complete.
...
the projection's own `path` — and a reader that re-derives it from this page has copied a convention
...
So the composition ships as code, in `@fossil-lang/corpus`, reading the manifest and nothing else:

```ts
const corpus = await openCorpus('/bench/1000000', { manifestFiles, wasmUrl });
const { vertexUrls, edgeUrls, complete, gaps } = corpus.tilesFor({ tiles, directions: ['src'] });
```

**The resolution reaches no network, and the module it runs in is up before the first address.** The
arithmetic is `fossil_graph::plan` at wasm32 — one implementation, not a second written in the
caller's language — and the boot is the price. It is an argument and not a call to sequence:
`wasmUrl` says where the module is, `openCorpus` awaits it, and that is why composing a URL out of
manifests already in hand is `await`ed. A caller holding an engine passes `query` instead and gets
the whole corpus from the same name; the manifests are the one thing every depth reads. Bytes, tile
