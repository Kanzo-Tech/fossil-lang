...
caller's language — and the boot is the price. It is not a call to sequence: `open` awaits it,
the module's `.wasm` is an asset the host's bundler already emitted, and that is why composing a
URL out of manifests already in hand is `await`ed. A caller holding an engine passes `query` instead and gets
