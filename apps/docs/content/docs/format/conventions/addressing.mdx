...
**Level `k` is the vertices whose `dense_id` is a multiple of `2^k`.** Over a Morton-ordered
`dense_id` that is one vertex per quadtree cell of depth `k`. Three things follow from the definition
alone, and none of them needs a byte on disk:

- **It nests, by construction.** `dense_id % 2^(k+1) == 0` is a strict subset of
  `dense_id % 2^k == 0`, so refining only ever ADDS and a vertex drawn once stays drawn at the same
  position.
...
- **There is no second `dense_id` space.** A level shares the payload's numbering, so a level tile is
  addressed by the payload's own shift plus `k` — level `k` keeps one id in `2^k`, so its tile of
  `chunk_size` rows spans that many times the ids: tile `j` covers
  `[j · chunk_size · 2^k, (j+1) · chunk_size · 2^k)`.
...
**Absent is a corpus and not a gap.** A type under the floor a writer spends bytes at — 64 tiles, at
which point the pyramid is 48% of the type, falling to 12.7% at 245 tiles — declares no `levels:` and draws the identical
picture.

Which levels a writer writes is a **policy**, not something a reader derives — but there is more than
one writer, so the policy is published as vectors and both of them execute it:
`fossil_sinks::manifest::VertexLevels::planned` through
`crates/fossil-sinks/tests/level_plan_vectors.rs`, and `apps/corpus/guards/fixture.mjs` through
`levelPlan` in `guards/arithmetic.mjs`. `a-level-is-the-predicate` then checks the files against the
predicate rather than against the plan, because a corpus is free to declare a different set as long
as what it wrote is what it says.

<VectorTable of="level_plan" />
