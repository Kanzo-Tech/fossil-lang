...
**The resolution reaches no network, and the module it runs in is loaded first.** The arithmetic is
`fossil_graph::plan` at wasm32 — one implementation, not a second written in the caller's language,
and the awaited init is the price. Bytes, tile caches, debounce, supersede-cancellation and sampling
stay in whatever draws. It refuses to compose
