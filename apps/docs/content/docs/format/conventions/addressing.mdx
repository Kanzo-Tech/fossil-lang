...
means listing a directory, and there is no listing over HTTP. Absent is `files`, which is what fossil
writes today. The row-group container's file is `tiles.parquet` in the set's own prefix, one per set,
and the addresses are published as vectors below.
...
<Callout type="warn" title="DuckDB cannot write a row group under 2,048 rows">
...
DuckDB at all — which is why the conformance corpus, at `chunk_size` 64, is the file-per-tile one and
the two-container comparison runs on a fixture at 4,096. It is a limit of that writer and not of the
format.
</Callout>
...
404 at runtime, in a browser, with no type error and nothing red. So the composition ships as code,
in `@fossil-lang/graph`, sharing nothing with the verbs but the manifest:
