...
graph LR
  ...
  subgraph B["one file, one row group per tile"]
    g["chunk0.parquet<br/>… rg 7 · rg 8 · rg 9 …<br/>+ one footer"]
  end
...
requests and does not save a byte.** The checker reports which container it found and refuses *both
at once*, because two ways to address the same rows is one too many and a reader that globs finds
both. Guard: `tile-of`, in whichever form the corpus uses.
