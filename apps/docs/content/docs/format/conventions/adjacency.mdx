...
```
by_source/chunk10.parquet, ordered by src_dense    the CSR a reader already has
...
graph LR
  ...
  E -->|"src_dense 41000 >> 12 = <b>10</b>"| S["<b>by_source/chunk10</b>.parquet<br/>every edge out of<br/>vertices 40960–45055"]
  E -->|"dst_dense 8300 >> 12 = <b>2</b>"| T["<b>by_target/chunk2</b>.parquet<br/>every edge into<br/>vertices 8192–12287"]
```
...
Every part of this comes out of the edge type's manifest — nothing is agreed out of band. Each
orientation is a [projection](/docs/format/conventions/addressing#projections-the-payload-and-the-levels-in-one-list)
at `scale: 1`: `path` is the URL, the one part a reader cannot compute; `aligned_by` is the
arithmetic, its shift `src_chunk_size` for the source-aligned one and `dst_chunk_size` for the other.
...
dst_chunk_size: 4096
projections:
- { path: by_source/, scale: 1, aligned_by: src, ordered: true, file_type: parquet }
- { path: by_target/, scale: 1, aligned_by: dst, ordered: true, file_type: parquet }
```
...
sequenceDiagram
  ...
  R->>S: GET {prefix}by_source/chunk{v >> shift(src_chunk_size)}.parquet
  S-->>R: filter src_dense = v → the far ends v points at
  R->>T: GET {prefix}by_target/chunk{v >> shift(dst_chunk_size)}.parquet
...
[design/discarded](/docs/design/discarded#the-corpus).
