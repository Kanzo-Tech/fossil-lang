...
description: A Parquet reader and the column names — the request sequence, the arithmetic between the requests, and the same read through @fossil-lang/corpus for a caller who would rather not compose a URL.
---
...
`type`, a `vertex_count`, a `prefix`, a `chunk_size` and optionally an `index:` and a `codes:` block; an edge
...
Arithmetic gives the tiles that *exist*. Which ones *intersect* has two routes and they answer the
same question at very different prices.

**The anchor, when the manifest declares one.** A `codes:` block names a small JSON document —
`vertex/Person/codes.json` — carrying the Morton code each tile begins and ends on, and the box the
positions were quantised against. Quantise the window's corners onto that box, decompose the
rectangle into Z-order ranges, and binary-search `lo`/`hi` for the tiles each range meets. That is
5,498 B and `JSON.parse` at a million vertices, against 226 kB and a Parquet reader — and it prunes
*tighter*, because a tile's code range describes an arc where its `x`/`y` box describes the arc's
rectangular hull. It is the only route open to a reader that cannot decode Parquet at all.
[Addressing](/docs/format/conventions/addressing#the-codes-are-the-anchor) has the document and its
guard.

**The footers, always.** The row-group statistics on `x` and `y` are bounding boxes, and selecting on
them is the whole of viewport culling. This is the route for a corpus that publishes no anchor, and
under `container: rowgroups` it is what turns the chosen tiles into byte ranges either way.
...
`@fossil-lang/corpus` composes all of it, and none of the arithmetic reaches the caller: no tiles, no
