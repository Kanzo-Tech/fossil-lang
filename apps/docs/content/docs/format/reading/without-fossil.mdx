...
the names from the count and the shift. It also assumes one tile per file — against the container
whose row groups are the tiles, drop the glob and the `file_name` column and read `row_group_id` as
the tile number, which is the cheaper of the two
([two containers, one address](/docs/format/conventions/addressing#two-containers-one-address)).
...
It refuses four things: a corpus declaring no `vertex_count`; the row-group container, refused **by
name** and never globbed; an edge label incident twice to one vertex type, which the addressing
cannot name unambiguously; and an `index:` with a `prefix` and no `ordered_by`.
