...
Arithmetic gives the tiles that *exist*. Which ones *intersect* comes from the footers: the
row-group statistics on `x` and `y` are bounding boxes, and selecting on them is the whole of
viewport culling. Under `container: rowgroups` the same statistics are what turn the chosen tiles
into byte ranges.
...
SELECT dst_dense AS far FROM read_parquet('edge/Person_knows_Person/by_source/chunk10.parquet')
 WHERE src_dense = 41000
UNION ALL
SELECT src_dense FROM read_parquet('edge/Person_knows_Person/by_target/chunk10.parquet')
 WHERE dst_dense = 41000;
```

Which path is which direction is in the edge type's manifest, one `projections` entry at `scale: 1`
per orientation: `aligned_by` names the column that addresses it, `path` names the directory. Do not
...
const corpus = await open('https://cdn.example.org/kg/2026-08', { query });

corpus.types                                   // what is inside
await corpus.extent('Person')                  // where it is, in coordinates
await corpus.rows({ x, y, w, h })              // vertices + edges in a rectangle
