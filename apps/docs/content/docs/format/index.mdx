...
| --- | --- | --- |
| [Identity](/docs/format/conventions/identity) | `dense_id` is a gapless `0..V−1` per vertex type, stored `uint32` and shifted as 64 bits; the identity is the subject IRI | a tile is a *range*, so its address is a shift |
