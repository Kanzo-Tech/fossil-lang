...
It is not a general condition, and `fossil check` is where it stops being one. Two things have to
hold of every conjunct, and each is a distinct refusal:

**It is an equality between two columns.** `on = A.date > B.since` is refused, and so is
`on = A.k == B.k or A.j == B.j`, and so is `on = str.lower(A.k) == B.k`. None of the three leaves a
key to plan against, so what each would run as is a nested loop over the product of two corpora —
and a corpus is the size at which that is not a slow program but a hung one.

**The two columns are one from each side.** `on = Purchase.id == Purchase.user_id` is well-typed,
names columns that exist, and never mentions `User` — so it holds or fails for whole relations
rather than for a pairing, and every order pairs with every user. That is a filter over the product,
not a join. It is [the sixth diagnostic](/docs/book/diagnostics#6-a-join-that-names-one-side-twice).
...
| `distinct` | nothing | one of each identical row |
| `union` | another relation | every row of both, under this pipeline's name |
| `group_by` | columns, and an aggregation per column it produces | one row per group |

Those six reach the surface today. The catalogue holds six more for a relation — `sort`, `take`,
`drop`, `count`, `map`, `flatten` — which the checker knows and the lowering does not yet handle;
[the standard library](/docs/book/stdlib) lists them. None is a special form: they are functions
with a relation on the left.
