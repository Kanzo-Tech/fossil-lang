...
writes `User.name` and not `Adults.name`. One verb is the exception, and only because it has to be:
a `union`'s rows come from two bindings with nothing in the data to say which, so neither name
identifies them and the derived name is what the body writes.
